...
!ifndef BUILD_UNINSTALLER
  ...
  Var DbUserInput
  Var DbPasswordInput
  Var DbPasswordShow      ; 비밀번호 보이기 체크박스
  Var DbDatabaseInput
  Var DbConfigDialog
  ...
  Function PortConfigPageCreate
    ...
    ${NSD_CreateLabel} 0 142u 100% 20u "※ 설정 파일: %APPDATA%\Aidot Express\.env  ·  로그: %APPDATA%\Aidot Express\log"
    Pop $0
  ...
  Function DbConfigPageCreate
    ...
    ; ★ v1.36.2 — 비밀번호를 눈으로 확인할 수 있게.
    ;   별표만 보이면 오타를 알 수 없고, 틀린 채로 설치하면 서버가 안 뜬다.
    ;   그때 원인을 찾기가 어렵다 — 여기서 한 번 보는 편이 훨씬 싸다.
    ${NSD_CreateCheckBox} 84% 114u 16% 12u "보기"
    Pop $DbPasswordShow
    ${NSD_OnClick} $DbPasswordShow DbPasswordToggle
    ...
    ${NSD_CreateLabel} 0 172u 100% 10u "※ 설정: %APPDATA%\Aidot Express\.env  ·  로그: %APPDATA%\Aidot Express\log"
    Pop $0
  ...
  ;
; 비밀번호 보이기/숨기기.
;   NSIS 에는 "이 입력칸의 마스킹만 끄기" 가 없다. EM_SETPASSWORDCHAR 로 마스크 문자를
;   0(없음) 또는 별표로 바꾸고 다시 그린다.
;
Function DbPasswordToggle
  Push $0
  Push $1
  ${NSD_GetState} $DbPasswordShow $0
  ${If} $0 == ${BST_CHECKED}
    StrCpy $1 0          ; 마스크 없음 — 글자가 그대로 보인다
  ${Else}
    StrCpy $1 9679       ; ● (U+25CF)
  ${EndIf}
  SendMessage $DbPasswordInput ${EM_SETPASSWORDCHAR} $1 0
  ; 다시 그리지 않으면 바뀐 마스크가 화면에 반영되지 않는다
  ShowWindow $DbPasswordInput ${SW_HIDE}
  ShowWindow $DbPasswordInput ${SW_SHOW}
  Pop $1
  Pop $0
FunctionEnd
...
Function DbTypeChanged
  ...
  !macro customInstall
    ...
    ${Else}
      ...
      FileClose $0
      DetailPrint "설정 저장: $1\.env"
      DetailPrint "로그 위치: $1\log   (문제가 생기면 이 폴더를 확인하세요)"
      CreateDirectory "$1\log"
    ${EndIf}
