...
function startServerProcess({ onLog, userDataPath } = {}) {
  return new Promise((resolve, reject) => {
    ...
    /* ★ v1.36.4 — 자식이 뱉은 것을 모아 둔다. 실패했을 때 오류 창에 보여 준다.
       콘솔이 없는 GUI 앱에서는 이것이 원인을 알 수 있는 유일한 통로다. */
    const recentOutput = [];
    const remember = (chunk) => {
      for (const line of String(chunk).split(/\r?\n/)) {
        if (!line.trim()) continue;
        recentOutput.push(line);
        if (recentOutput.length > 400) recentOutput.shift();
      }
    };
    ...
    const timer = setTimeout(() => {
      proc.kill('SIGKILL');
      /* 시간 초과일 때도 그동안 자식이 뱉은 것을 보여 준다 —
         "30000ms 내에 준비되지 않았습니다" 한 줄만으로는 아무것도 알 수 없다. */
      const err = new Error(`서버가 ${STARTUP_TIMEOUT_MS}ms 내에 준비되지 않았습니다`);
      err.reason = err.message;
      err.output = recentOutput.slice(-200);
      err.envPath = userDataPath ? path.join(userDataPath, '.env') : '';
      err.logPath = process.env.LOG_DIR || (userDataPath ? path.join(userDataPath, 'log') : '');
      reject(err);
    }, STARTUP_TIMEOUT_MS);
    ...
    proc.once('message', (msg) => {
      ...
      if (msg && msg.type === 'error') {
        /* ★ v1.36.4 — 자식 출력과 경로를 **그대로 실어 보낸다.**
           예전에는 message 한 줄만 만들어 넘겨, supervisor 가 애써 모은 스택이
           여기서 버려졌다. 오류 창이 그것을 보여 줘야 한다. */
        const err = new Error(msg.reason || msg.message || '서버 기동에 실패했습니다');
        err.reason = msg.reason || msg.message || '';
        err.output = (Array.isArray(msg.output) && msg.output.length)
          ? msg.output
          : recentOutput.slice(-200);
        err.envPath = msg.envPath || '';
        err.logPath = msg.logPath || '';
        reject(err);
      } else {
