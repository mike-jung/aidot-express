...
export default class AdminAuthController {
  ...
  async signup(params, req, res) {
    ...
    if (!allowed.ok) {
      this.log.warn(`[admin] signup 거부 (${allowed.reason}) ip=${req.ip} username=${params.username}`);
      res.status(403).json({
        code: 403,
  ...
  async _signupAllowed(req) {
    ...
    if (m) {
      try {
        ...
        if (payload.realm === REALMS.ADMIN && payload.role === 'admin') return { ok: true, reason: `admin:${payload.username || payload.sub}` };
