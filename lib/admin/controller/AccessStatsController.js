...
export default class AccessStatsController {
  ...
  async stream(params, req) {
    this.log.info(`[access-stream] subscribed user=${req.user?.username ?? '-'}`);
  ...
  async forceEndSession(params, req) {
    ...
    this.log.warn(`[access] force-end session requested: sessionId=${sessionId} by=${actorUsername || '(unknown)'}`);
