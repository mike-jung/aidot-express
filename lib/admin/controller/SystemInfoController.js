...
import { resolveDirs, workspaceRoots, canWrite, runMode } from '../../../src/core/appPaths.js';
...
export default class SystemInfoController {
  ...
  async setFeatures(params, req) {
    ...
    this.log.warn(`[system] feature settings changed by ${req.user?.username || '-'}`);
    return { data };
  }
  ...
  async uiFlags() {
    return {
      data: {
        ...
        /* ★ v1.39.0 — 지금 어떤 방식으로 도는지. 설치본 서버를 켜 둔 채 콘솔만
           `npm start` 로 띄우면 화면이 똑같아 어느 쪽인지 알 수 없다. */
        runMode: runMode(),
        /* ★ v1.36.3 — 어느 폴더에서 읽고 어디에 쓰는지 콘솔에서 볼 수 있게.
           설치본은 Program Files 에 놓여 쓸 수 없다 — 그 사실이 보이지 않으면
           "왜 저장이 안 되지" 를 한참 헤맨다. */
        workspaceDirs: workspaceRoots().map((d) => ({ path: d, writable: canWrite(d) })),
        loadDirs: {
          controllers: resolveDirs('controllers'),
          services: resolveDirs('services'),
          sql: resolveDirs('sql'),
        },
