 ...
 */
import {
  Controller, GetMapping, PutMapping, PostMapping, Autowired, Auth, Roles, Log,
...
export default class ConfigController {
  ...
  @Auth()
  transport() { return { data: this.httpsService.metadata() }; }

  @GetMapping('/https')
  @Roles('admin')
  https() { return { data: this.httpsService.getSettings() }; }

  @PutMapping('/https')
  @Roles('admin')
  saveHttps(params, req) {
    const data = this.httpsService.saveSettings(req.body);
    this.log.warn('[https] Configuration saved; restart the complete app/service to apply changes');
    return { data };
  }

  @PostMapping('/https/certificate')
  @Roles('admin')
  async generate(params, req) { return { data: await this.httpsService.generate(req.body) }; }

  @GetMapping('/https/ca')
  @Roles('admin')
  ca() { return { data: this.httpsService.caCertificate() }; }
