...
export default class ConfigController {
  ...
  async setWorkspace(params, req) {
    ...
    this.log.warn(`[config] 작업 폴더 변경 by ${req.user?.username || '-'}: "${data.configured || '(사용 안 함)'}"`);
    return { data };
  }
  ...
  async setAppSchema(params, req) {
    ...
    this.log.warn(`[config] 업무 테이블 스키마 변경 by ${req.user?.username || '-'}: "${data.configured || '(접속 스키마)'}"`);
    return { data };
  }
  ...
  async setEai(params, req) {
    ...
    this.log.warn(`[config] EAI 메뉴 ${data.enabled ? '켬' : '끔'} by ${req.user?.username || '-'}`);
    return { data };
  }
