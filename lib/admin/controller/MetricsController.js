...
export default class MetricsController {
  ...
  async stream(params, req) {
    ...
    this.log.info(`[metrics-stream] subscribed user=${req.user?.username ?? '-'} ${channel}`);
  ...
  @Auth()
  async routes(params) {
    const windowSec = clampInt(params.windowSec, 10, 86400, 60);
    ...
    try {
      ...
      this.log.error(`[metrics.routes] failed params=${JSON.stringify({windowSec,limit,sortBy,includeSeries})}: ${e.message}\n${e.stack}`);
  ...
  async routeSeries(params) {
    ...
    try {
      ...
      this.log.error(`[metrics.routeSeries] failed key=${key}: ${e.message}\n${e.stack}`);
