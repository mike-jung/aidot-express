...
export default class SqlMetaService {
  ...
  async generate(input) {
    ...
    if (columns.length === 0 && input.tableName) {
      try {
        ...
        this.log.warn(`[meta:sql] generate: 컬럼 자동 조회 중 예외 — ${e.message}. SELECT * fallback`);
      }
    }
  ...
  async create(input) {
    ...
    try {
      ...
      this.log.info(`[meta:sql] 동적 등록: ${r.fileName} (${r.count} 문)`);
    } catch (e) {
      this.log.error(`[meta:sql] 동적 등록 실패: ${e.message}`);
    }
  ...
  async update(id, input) {
    ...
    try {
      const r = sqlRegistry.registerFile(input.name, finalContent);
      this.log.info(`[meta:sql] 동적 갱신: ${r.fileName} (${r.count} 문)`);
    } catch (e) {
      this.log.error(`[meta:sql] 동적 갱신 실패: ${e.message}`);
    }
  ...
  async testQuery(sqlFile, queryName, params = {}, sqlBody = null) {
    ...
    try {
      ...
      if (sqlBody && typeof sqlBody === 'string' && sqlBody.trim()) {
        ...
        this.log.info(`[sql-test] 즉시실행(sqlBody): ${sqlFile || '(new)'}:${queryName || '(adhoc)'}`);
      } else {
        ...
        this.log.info(`[sql-test] 레지스트리 조회 실행: ${key}`);
      }
      ...
      this.log.info(`[sql-test] 감지 파라미터: ${JSON.stringify(usedParams)}`);
      ...
      this.log.info(`[sql-test] 실행 완료: ${elapsed}ms, rows=${Array.isArray(result.rows) ? result.rows.length : 'N/A'}, rowsAffected=${result.rowsAffected ?? 'N/A'}`);
      ...
      this.log.info(`[sql-test] 정제 후 rows: ${rows.length}건`);
      ...
      const elapsed = Date.now() - startTime;
      this.log.error(`[sql-test] 에러: ${e.message}`);
      return {
        success: false,
        sqlFile,
        queryName,
  ...
  async getTableColumns(tableName) {
    ...
    try {
      ...
      this.log.error(`[table-columns] 에러: ${e.message}`);
