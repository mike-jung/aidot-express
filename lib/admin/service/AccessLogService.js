...
export default class AccessLogService {
  ...
  start() {
    if (!this._flushTimer) {
      this._flushTimer = setInterval(() => {
        ...
        if (!isActive()) return;
        this._flushQueue().catch((e) => this.log.warn(`[access] flush failed: ${e.message}`));
      }, FLUSH_INTERVAL_MS);
    ...
    if (!this._retentionTimer) {
      ...
      this._retentionTimer = setInterval(() => {
        ...
        if (!isActive()) return;
        this._sweepOld().catch((e) => this.log.debug(`[access] sweep failed: ${e.message}`));
      }, RETENTION_SWEEP_MS);
      this._retentionTimer.unref?.();
    }
    this.log.info('[access] AccessLogService started (batched flush, retention sweep)');
  }
    ...
    try {
      for (const entry of batch) {
        try {
          ...
          this.log.debug(`[access] insert failed: ${e.message}`);
        }
      }
    } finally {
      this._flushing = false;
  ...
  async _sweepOld() {
    ...
    for (const key of ['deleteOldAccessLogs', 'deleteOldLoginEvents', 'deleteOldSessions']) {
      try {
        ...
        this.log.debug(`[access] ${key} failed: ${e.message}`);
      }
    }
    ...
    if (Number.isFinite(maxRecords) && maxRecords > 0) {
      try {
        ...
        if (total > maxRecords) {
          ...
          this.log.info(`[access] maxRecords(${maxRecords}) exceeded → deleting the oldest ${excess} (was total=${total})`);
        }
      } catch (e) {
        if (isMissingTableError(e)) { this._tablesMissing = true; return; }
        this.log.debug(`[access] maxRecords cleanup failed: ${e.message}`);
      }
    }
  }
  ...
  async recordLogin({ userId, username, sessionKind = 'user', ip = null, userAgent = null } = {}) {
    try {
      ...
      this.log.debug(`[access] recordLogin failed: ${e.message}`);
  ...
  async recordLoginFailed({ userId = null, username, sessionKind = 'user', ip = null, userAgent = null } = {}) {
    try {
      ...
      if (isMissingTableError(e)) this._tablesMissing = true;
      else this.log.debug(`[access] recordLoginFailed failed: ${e.message}`);
    }
  }
  ...
  async recordLogout({ userId, username, sessionKind = 'user', sessionId = null, all = false, ip = null, userAgent = null } = {}) {
    ...
    try {
      ...
      if (isMissingTableError(e)) this._tablesMissing = true;
      else this.log.debug(`[access] recordLogout failed: ${e.message}`);
    }
  }
  ...
  async forceEndSession({ sessionId, actorUsername = null } = {}) {
    ...
    try {
      ...
      try {
        ...
        this.log.warn(`[access] forceEndSession could not revoke the token (the session was ended): ${e.message}`);
      }
  ...
  async getSummaryByController({ fromMs, toMs, limit = 200, offset = 0, excludeAdmin = false, username = null } = {}) {
    ...
    try {
      ...
      this.log.warn(`[access] per-controller summary failed: ${e.message}`);
