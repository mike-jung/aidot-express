...
export default class MetricsService {
  ...
  async start() {
    ...
    try {
      ...
      if (isMissingTableError(e)) {
        ...
        this.log.warn(`[metrics] threshold 초기 로드 실패(무시): ${e.message}`);
      }
    }
    ...
    if (!this._flushTimer) {
      this._flushTimer = setInterval(() => {
        ...
        if (!isActive()) return;
        this._flushToDb().catch((e) => this.log.warn(`[metrics] flush 실패: ${e.message}`));
      }, FLUSH_INTERVAL_MS);
    ...
    if (!this._evalTimer) {
      this._evalTimer = setInterval(() => {
        ...
        if (!isActive()) return;
        this._evaluate().catch((e) => this.log.warn(`[metrics] evaluate 실패: ${e.message}`));
      }, EVAL_INTERVAL_MS);
    ...
    if (!this._retentionTimer) {
      this._retentionTimer = setInterval(() => {
        /* ★ v1.14.0 — 액티브에서만. 두 대가 동시에 돌면 보관 정리가 두 번 일어난다
           (삭제 중복 · 경보 중복 발송). HA 가 꺼져 있으면 isActive() 는 항상 true 다. */
        if (!isActive()) return;
        this._sweepOldSamples().catch((e) => this.log.warn(`[metrics] sweep 실패: ${e.message}`));
      }, RETENTION_SWEEP_MS);
  ...
  async _flushToDb() {
    ...
    for (const s of samples) {
      try {
        ...
        this.log.debug(`[metrics] insertSample 실패 (${s.kind}.${s.metric}): ${e.message}`);
      }
    }
  }
  ...
  async _sweepOldSamples() {
    ...
    try {
      ...
      this.log.debug(`[metrics] deleteOldSamples 실패: ${e.message}`);
    }
    ...
    if (Number.isFinite(maxRecords) && maxRecords > 0) {
      try {
        ...
        if (total > maxRecords) {
          ...
          this.log.info(`[metrics] maxRecords(${maxRecords}) 초과 → 오래된 ${excess}건 삭제 (이전 total=${total})`);
        }
      } catch (e) {
        if (isMissingTableError(e)) { this._tablesMissing = true; return; }
        this.log.debug(`[metrics] maxRecords 정리 실패: ${e.message}`);
      }
    }
  }
  ...
  async _reloadThresholds() {
    try {
      ...
      if (this._tablesMissing) {
        // 복구 성공
        this._tablesMissing = false;
        this.log.info('[metrics] 모니터링 테이블 접근 복구됨 — DB 저장/알림 활성');
      }
    } catch (e) {
      if (isMissingTableError(e)) {
        this._tablesMissing = true;
      ...
      if (isOver) {
        if (!state) {
          ...
          if (!state.alertId && held >= t.durationSec) {
            try {
              ...
              this.log.warn(`[metrics] alert 생성 실패: ${e.message}`);
            }
        ...
        if (state?.alertId) {
          try {
            ...
            this.log.debug(`[metrics] resolve 실패: ${e.message}`);
          }
        }
  ...
  async getCurrent({ windowSec = 60, includeAlerts = true, topRoutes = 5 } = {}) {
    ...
    try {
      ...
      this.log.error(`[metrics.getCurrent] getSnapshot 실패: ${e.message}\n${e.stack}`);
      throw Object.assign(new Error(`getSnapshot failed: ${e.message}`), { status: 500 });
    }
    try {
      series = metrics.getSeries(Number(windowSec) || 60);
    } catch (e) {
      this.log.error(`[metrics.getCurrent] getSeries 실패: ${e.message}\n${e.stack}`);
    ...
    try {
      ...
      this.log.error(`[metrics.getCurrent] getRouteSummary 실패: ${e.message}\n${e.stack}`);
    ...
    if (includeAlerts && !this._tablesMissing) {
      try {
        ...
        if (isMissingTableError(e)) {
          ...
          this.log.warn(`[metrics.getCurrent] findActiveAlerts 실패 (무시하고 계속): ${e.message}`);
        }
      }
    }
    return out;
