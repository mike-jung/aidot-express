...
export default class MetricsService {
  ...
  async getRouteStats({ windowSec = 60, limit = 100, sortBy = 'count', includeSeries = true } = {}) {
    ...
    if (!includeSeries) return { rows: summary, windowSec, coverage: metrics.getRouteCoverage(windowSec) };
    const rows = summary.map((r) => {
      const s = metrics.getRouteSeries(r.key, windowSec);
      return { ...r, intervalSec: s.intervalSec || 1, series: s.series.map((b) => ({ tsSec: b.tsSec, count: b.count, avgMs: b.avgMs, intervalSec: b.intervalSec || 1 })) };
    });
    return { rows, windowSec, coverage: metrics.getRouteCoverage(windowSec) };
  }
