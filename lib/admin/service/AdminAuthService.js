...
export default class AdminAuthService {
  ...
  async signup(input) {
    ...
    try {
      ...
      this.log.info(`[admin] 가입 성공 id=${id} username=${input.username}`);
      return { id };
    } catch (e) {
  ...
  async login(input, meta = {}) {
    ...
    try {
      ...
      this.log.error(`[admin] DB 접속 실패 during login: ${msg}`);
    ...
    if (user?.locked_until && new Date(user.locked_until) > new Date()) {
      this.log.warn(`[admin] 잠긴 계정 로그인 시도 id=${user.id}`);
    ...
    if (!user || !passwordOk || user.status !== 'active') {
      ...
      this.log.warn(`[admin] 로그인 실패 username=${input.username}`);
      // 접속 통계: 로그인 실패 기록
      try {
  ...
  async rotate(rawToken, meta = {}) {
    ...
    if (rt.revoked_at) {
      this.log.error(`[admin] refresh token 재사용 감지 user=${rt.user_id} family=${rt.family_id}`);
      await db.execute(this.authSql.get('revokeTokenFamily'), { family_id: rt.family_id });
    ...
    return db.transaction(async (tx) => {
      ...
      await tx.execute(this.authSql.get('revokeRefreshToken'), {
        id: rt.id, replaced_by_id: newId,
      });
      const accessToken = signAccessToken({ id: user.id, role: user.role, username: user.username }, { realm: REALMS.ADMIN });
      return {
        accessToken,
