...
export function ensurePublisher(metricsService, { windowSec, intervalMs, topRoutes }) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      logger.warn(`[metrics-stream] snapshot failed (${channel}): ${e.message}`);
  ...
  logger.info(`[metrics-stream] publishing started channel=${channel} (${intervalMs}ms interval)`);
  return channel;
}
...
export function stopPublisher(channel) {
  ...
  logger.info(`[metrics-stream] publishing stopped channel=${channel} (no subscribers)`);
  return true;
}
