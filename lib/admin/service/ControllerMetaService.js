...
import { analyzeControllerFlow } from './controllerFlow.js';
...
import { writeAndLoadCode } from './codeFileUpdate.js';
...
export default class ControllerMetaService {
  ...
  async update(id, meta, customCode) {
    ...
    // Verify the rename before writing; the old source and router remain until success.
    ...
    let registered = false;
    await writeAndLoadCode({ filePath, previousFilePath: oldPath, code, meta: candidateMeta, load: async () => {
      if (_app) {
        await loadSingleControllerFile(_app, filePath, { replacesFile: oldPath });
        markLoaded('controller', filePath);
        registered = true;
      } else {
   ...
   * ========================================================= */
  async analyzeFlow(id) {
    const ctrl = await this.findById(id);
    if (!ctrl) throw Object.assign(new Error(`컨트롤러 없음: ${id}`), { status: 404 });
    return analyzeControllerFlow(ctrl, {
      findServiceFile: name => this._findServiceFile(name),
      findSqlFile: name => this._findSqlFile(name),
      parseSql: source => this._parseSqlQueries(source),
    });
  }

  async analyzeParameters(id) {
    const flow = await this.analyzeFlow(id);
    return { ...flow, note: flow.warnings.join('\n') || null };
  }
  ...
  _findServiceFile(className) {
    ...
    const direct = findExistingFile('services', ['js', 'ts', 'mjs'].map(ext => `${className}.${ext}`));
    if (direct) return direct;
    // @Service 이름이 파일명과 다른 수동 작성 서비스도 찾는다.
    for (const dir of existingDirs('services').reverse()) {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (!entry.isFile() || !/\.(m?js|ts)$/.test(entry.name)) continue;
        const file = path.join(dir, entry.name);
        const source = fs.readFileSync(file, 'utf8');
        if (/@Service\(\s*['"]([^'"]+)['"]/.exec(source)?.[1] === className) return file;
      }
    }
    return null;
  }
  ...
  _parseSqlQueries(src) {
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      out.push({ name, type, inputParams, outputColumns, text: cleanBody });
    }
    return out;
  }
