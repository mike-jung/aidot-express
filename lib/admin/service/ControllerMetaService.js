...
export default class ControllerMetaService {
  ...
  async create(meta, customCode) {
    ...
    if (_app) {
      try {
        ...
        this.log.info(`[meta:controller] 동적 로딩 성공: ${meta.name} (${meta.basePath})`);
        registered = true;
      } catch (e) {
        this.log.error(`[meta:controller] 동적 로딩 실패: ${e.message}`);
      ...
      this.log.warn(`[meta:controller] _app 이 null 입니다. server.js 에서 setApp(app) 호출 여부를 확인하세요. 파일은 저장되었지만 서버 재시작 전까지 라우트에 접근할 수 없습니다.`);
    }
  ...
  async update(id, meta, customCode) {
    ...
    if (_app) {
      ...
      try {
        ...
        this.log.error(`[meta:controller] update 후 동적 로딩 실패: ${e.message}`);
      ...
      this.log.warn(`[meta:controller] _app 이 null. server.js 에서 setApp(app) 호출 필요. 서버 재시작 전까지 라우트 접근 불가.`);
    }
  ...
  async _syncServiceMethods(meta, controllerCode = '') {
    try {
      ...
      if (regenerated && regenerated === source) {
        ...
        this.log.info(`[meta:controller] ${serviceName} 에 메서드 추가(재생성): ${missing.join(', ')}`);
      ...
      this.log.info(`[meta:controller] ${serviceName} 에 메서드 추가(덧붙임): ${missing.join(', ')}`);
      return { service: serviceName, added: missing, how: 'appended' };
    } catch (e) {
      this.log.warn(`[meta:controller] 서비스 메서드 동기화 실패(무시): ${e.message}`);
  ...
  _scanAllControllers() {
    ...
    try {
      ...
      this.log.warn(`[ControllerMeta] legacy 컨트롤러 수집 실패: ${e.message}`);
    }
  ...
  _buildLegacyControllerInfo(meta) {
    ...
    const filePathRel = meta.file
      ? path.relative(projectRoot, meta.file).replace(/\\/g, '/')
      : null;
  ...
  async analyzeParameters(id) {
    ...
    for (const svc of services) {
      ...
      const svcInfo = {
        ...
        filePath: svcFile ? path.relative(projectRoot, svcFile).replace(/\\/g, '/') : null,
        sqls: [],
      };
      ...
      if (svcFile && fs.existsSync(svcFile)) {
        ...
        for (const binding of sqlBindings) {
          ...
          result.sqlFiles.push({
            ...
            filePath: path.relative(projectRoot, sqlFile).replace(/\\/g, '/'),
            queries,
          });
        }
      }
