...
import { readMeta, deleteMeta } from './metaStorage.js';
...
export default class ServiceMetaService {
  ...
  async create(input, customCode) {
    ...
    await writeAndLoadCode({ filePath, code, meta: candidateMeta, load: async () => {
      await loadSingleServiceFile(filePath);
    } });
    markLoaded('service', filePath);
    this.log.info(`[meta:service] registered: ${input.name}`);
