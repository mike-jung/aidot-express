 ...
 *    Electron     : admin / admin1234, 초기 안내 창에서 복사·임의 생성 가능
 *    production   : 독립 서버는 환경변수가 없으면 임의 비밀번호 사용
 *    공통         : ADMIN_INITIAL_PASSWORD 우선, 기존 계정은 변경하지 않음
 *
...
import { DEFAULT_ADMIN_PASSWORD, initialPassword } from '../../../src/core/bootstrapPassword.cjs';

const ARGON2_OPTS = { type: argon2.argon2id, memoryCost: 19456, timeCost: 2, parallelism: 1 };

export const DEV_DEFAULT_ADMIN = Object.freeze({ username: 'admin', password: DEFAULT_ADMIN_PASSWORD });
...
export async function ensureInitialAdmin(logger) {
  ...
  const { password, source, disclose, generated } = initialPassword({ production: config.env === 'production' });
  ...
  const credentialPath = disclose ? saveInitialCredential({ username, password }) : null;
