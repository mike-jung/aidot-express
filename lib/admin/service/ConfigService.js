...
import { workspaceRoots, writeDirFor } from '../../../src/core/appPaths.js';
...
const SECRET_KEY_RE = /(SECRET|PASSWORD|PASSWD|PASSPHRASE|TOKEN|PRIVATE_KEY|API_KEY|MASTER|CREDENTIAL|_KEY$|^KEY_)/i;
...
export default class ConfigService {
  ...
  getWorkspace() {
    ...
    const directoryExists = dir => { try { return fs.statSync(dir).isDirectory(); } catch { return false; } };
    let writePath = null;
    try { writePath = writeDirFor('controllers', { create: false }); } catch { /* 쓰기 경로 없음 */ }
    const abs = roots.find(root => path.join(root, 'controller') === writePath) || roots.at(-1) || null;
    ...
    const rootDetails = roots.map(root => ({
      path: root, exists: directoryExists(root), active: root === abs,
      folders: kinds.map(kind => {
        const dir = path.join(root, SUB[kind]);
        const exts = kind === 'sql' ? ['.sql'] : kind === 'scenarios' ? ['.json'] : ['.js', '.mjs', '.ts'];
        return { kind, path: dir, exists: directoryExists(dir), count: countIn(dir, exts) };
      }),
    }));
    const folders = kinds.map(kind => {
      const matches = rootDetails.flatMap(root => root.folders.filter(folder => folder.kind === kind));
      let target = null;
      try { target = writeDirFor(kind, { create: false }); } catch { /* 읽기 전용 */ }
      return { kind, sub: SUB[kind], path: target,
        exists: matches.some(folder => folder.exists), count: matches.reduce((total, folder) => total + folder.count, 0) };
    });
    ...
    return {
      ...
      activePath: abs,
      roots: rootDetails,
      installedWorkspace: !!(process.env.AIDOT_DATA_DIR || process.env.ELECTRON_USER_DATA),
      exists: !!(abs && directoryExists(abs)),
      projectRoot,
