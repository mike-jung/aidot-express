...
import { projectRoot } from '../../../src/core/appPaths.js';   // ★ v1.12.0
...
export default class ConfigService {
  ...
  getWorkspace() {
    ...
    const folders = kinds.map((kind) => {
      ...
      return {
        kind, sub: SUB[kind],
        path: dir ? path.relative(projectRoot, dir).replace(/\\/g, '/') : null,
  ...
  async setWorkspace({ dir, create = true } = {}) {
    ...
    if (raw) {
      if (/\0/.test(raw)) throw Object.assign(new Error('경로에 쓸 수 없는 문자가 있습니다.'), { status: 400 });
      const abs = path.resolve(projectRoot, raw);
      const rel = path.relative(projectRoot, abs);
      if (!rel || rel.startsWith('..') || path.isAbsolute(rel)) {
        throw Object.assign(new Error('프로젝트 폴더 안의 경로만 쓸 수 있습니다 (예: workspace, my-app).'), { status: 400 });
      }
      if (['src', 'lib', 'node_modules', 'admin-client', 'public', 'log', 'data'].includes(rel.split('/')[0])) {
        throw Object.assign(new Error(`${rel.split('/')[0]} 아래는 프레임워크 폴더라 작업 폴더로 쓸 수 없습니다.`), { status: 400 });
      }
      if (!fs.existsSync(abs)) {
    ...
    this.log.warn(`[config] 작업 폴더 변경: "${raw || '(사용 안 함)'}" — 이미 로드된 라우트는 서버를 다시 켜야 정리됩니다`);
  ...
  async setAppSchema({ schema, create = false } = {}) {
    ...
    this.log.warn(`[config] 업무 테이블 스키마 변경: "${raw || '(접속 스키마 사용)'}"`);
  ...
  getEaiFlag() {
    return {
      ...
      envPath: resolveEnvPath(),
      note: '켜면 [EAI 컨트롤러 생성]·[EAI 템플릿 편집]·[EAI 약어 사전] 메뉴가 나타납니다. 이미 만든 EAI 컨트롤러는 이 설정과 무관하게 그대로 동작합니다.',
    };
  }
  ...
  async setEaiFlag({ enabled } = {}) {
    ...
    this.log.warn(`[config] EAI 메뉴 ${on ? '켬' : '끔'}`);
  ...
  getHaConfig() {
    ...
    return {
      ...
      timings: { ...(c.timings || {}) },
      envPath: resolveEnvPath(),
      note: '바꾼 값은 서버를 다시 켜야 적용됩니다 — 에이전트가 기동할 때 설정을 읽습니다.',
    };
  }
  ...
  async setHaConfig(input = {}) {
    ...
    const wMode = String(input.witness?.mode || 'none').toLowerCase();
    if (!['none', 'http', 'file'].includes(wMode)) throw Object.assign(new Error('witness 방식은 none · http · file 중 하나입니다.'), { status: 400 });
    if (wMode === 'http' && !String(input.witness?.url || '').trim()) throw Object.assign(new Error('witness 주소가 필요합니다.'), { status: 400 });
    if (wMode === 'file' && !String(input.witness?.path || '').trim()) throw Object.assign(new Error('witness 파일 경로가 필요합니다.'), { status: 400 });
    ...
    fs.writeFileSync(envPath, text, 'utf8');
    this.log.warn(`[config] 이중화 설정 변경 — mode=${mode} node=${pairs.HA_NODE_ID} witness=${pairs.HA_WITNESS_MODE}`);
  ...
  async setFeatures(input = {}) {
    ...
    this.log.warn(`[config] 기능 설정 변경 — backup=${next.backup} secureColumns=${next.secureColumns} ha=${next.ha}`);
  ...
  async writeFile({ id, content } = {}) {
    ...
    if (fs.existsSync(abs)) {
      ...
      catch (e) { this.log.warn(`[config] 백업 실패 (무시): ${e.message}`); backupPath = null; }
    }
    ...
    this.log.info(`[config] 저장: ${info.relPath} (${content.length} bytes)${backupPath ? ' + 백업' : ''}`);
