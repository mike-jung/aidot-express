...
import { clearInitialCredential } from '../../../src/core/bootstrapCredential.cjs';
...
export default class AdminUserService {
  ...
  async create({ name, username, email, password, role = 'admin', status = 'active' }) {
    ...
    this.log.info(`[admin] user added: ${username} (role=${role}, must change password at first sign-in)`);
  ...
  async update(id, { name, email, role, status }) {
    ...
    if (nextStatus !== 'active' && existing.status === 'active') {
      ...
      catch (e) { this.log.warn(`[admin] could not revoke sessions (ignored): ${e.message}`); }
      this.log.info(`[admin] user disabled → sessions cut immediately: id=${id}`);
    } else if (nextStatus === 'active' && existing.status !== 'active') {
      unrevokeUser(id);
    }
    this.log.info(`[admin] user updated: id=${id}`);
    return { id };
  }
  ...
  async changePassword(id, { currentPassword, newPassword, skipCurrentCheck = false } = {}) {
    ...
    // 비밀번호 변경 시 기존 refresh 세션 전부 폐기 (탈취된 세션 차단). 계정 버전 변경으로 이전 access token 도 거부된다.
    try {
      await db.execute(sqlRegistry.getFile('admin_auth').get('revokeAllUserTokens'), { user_id: id });
    } catch (e) { this.log.warn(`[admin] could not revoke sessions (ignored): ${e.message}`); }
    try { clearInitialCredential(who[0]?.username); } catch { this.log.warn('[admin] Remove the local initial-credentials file after password setup'); }
    this.log.info(`[admin] password changed: id=${id}`);
  ...
  async remove(id, opts = {}) {
    ...
    this.log.info(`[admin] user deleted: id=${id} (${user.username})`);
    return { id };
  }
