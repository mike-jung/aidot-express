...
import { loadSingleControllerFile, loadSingleServiceFile, listRegisteredControllers } from '../../../src/core/controllerLoader.js';
...
/**
 * ★ v1.38.1 — **만든 쪽에서도 "올라옴" 으로 표시할 수 있게** 밖으로 낸다.
 *
 *  콘솔에서 SQL 을 만들면 목록에 "안 올라옴" 으로 보이고 손으로 한 번 더 눌러야 했다.
 *  실제로는 SqlMetaService 가 `sqlRegistry.registerFile()` 로 **이미 등록하고 있었다** —
 *  등록은 됐는데 이 표가 그것을 몰랐을 뿐이다. 화면만 틀렸던 셈이다.
 *
 *  `_mark` 는 인스턴스 메서드라 만드는 쪽(SqlMetaService 등)에서 부를 수 없었다.
 *  같은 Map 을 쓰는 함수를 내보내 두 곳이 같은 사실을 보게 한다.
 *
 *  ⚠ 파일을 쓴 **뒤에** 불러야 한다 — mtime 을 기준으로 판정하기 때문이다.
 */
export function markLoaded(kind, file) {
...
export default class WorkspaceLoadService {
   ...
   */
  _mark(kind, file) { markLoaded(kind, file); }
  ...
  seedLoadedAtBoot() {
    ...
    if (meta) this.log.info(`[workspace] meta files created at boot: ${meta}`);
  }
  ...
  _ensureMeta(kind, name, file) {
    ...
    try {
      ...
      if (!meta) return null;
      /* ⚠ writeMeta 는 async 다. await 하지 않으면 거부(EPERM)가 이 try 밖으로 새어
         **부팅을 죽인다.** 실제로 그렇게 죽었다 — 스택에 _ensureMeta 가 있었다. */
      Promise.resolve(writeMeta(file, meta)).catch((e) => {
        this.log.warn(`[workspace] could not save meta — ${name}: ${e.message}`);
      });
      this.log.info(`[workspace] meta created — ${kind}: ${name}`);
      return meta;
    } catch (e) {
      /* meta 를 못 만들어도 **올리기 자체는 성공한 것**이다. 조용히 넘어가지 말고 남긴다. */
      this.log.warn(`[workspace] could not create meta — ${name}: ${e.message}`);
      return null;
    }
  }
  ...
  async loadOne(kind, name) {
    ...
    this.log.warn(`[workspace] loaded one — ${kind}: ${name}`);
  ...
  async loadAll() {
    ...
    for (const s of before.services) {
      try {
        await loadSingleServiceFile(s.file);
        this._mark('service', s.file);
        this._ensureMeta('service', s.name, s.file);
        done.services += 1;
      } catch (e) {
        errors.push({ kind: 'service', name: s.name, message: e.message });
      }
    }
    for (const c of before.controllers) {
      try {
        await loadSingleControllerFile(await this._resolveApp(), c.file);
        this._mark('controller', c.file);
    ...
    this.log.warn(`[workspace] loaded hand-placed files — SQL ${done.sql} · services ${done.services} · controllers ${done.controllers}`
