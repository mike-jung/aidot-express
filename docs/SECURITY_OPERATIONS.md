...
Applies to 1.43.0. Read the upgrade guide before changing an existing deployment.

## Deployment settings

Run a supported Node.js LTS release satisfying `engines`; Node 24 was used for this
review. Install with `npm ci`, including the separate `admin-client` lockfile.
...
Back up the database, workspace, environment configuration and encryption keys through
separate protected channels before upgrading. Protect keys independently from data.
Restore now rejects unsafe names, duplicate paths, symlink entries, existing symlink
parents, files above 16 MiB and a total declared uncompressed size above 100 MiB.
Compressed input and entry-count limits also apply. Restore into an OS-protected
working directory; local processes that can concurrently replace directory entries
are outside the restore guard's isolation boundary.
...
The review's root `npm audit` reports a moderate `adm-zip` advisory,
[GHSA-vwc7-r8mq-g2x9](https://github.com/advisories/GHSA-vwc7-r8mq-g2x9).
The application uses guarded reads and writes rather than `extractAllTo` and now
rejects pre-existing symlink parents. The package-level advisory remains; it is not
waived or represented as fixed upstream. Do not apply the suggested downgrade to an
older release without checking other archive vulnerabilities. Track an upstream
fixed release or replace the ZIP reader and rerun adversarial restore tests.
