...
export async function runMigrations(opts) {
  ...
  if (adapter !== 'mariadb' && adapter !== 'sqlite') {
    logger.info(`[migration] adapter=${adapter} → skipping automatic migrations`);
    return;
  }
  ...
  for (const { absPath, fileName, relPath } of files) {
    if (applied.has(fileName)) {
      ...
      logger.warn(`[migration] ${missing.join(', ')} not found `
        + `— 적용 기록은 있지만 ${relPath} 를 다시 적용합니다.`);
    }
    logger.info(`[migration] applying: ${relPath}`);
    const startedAt = Date.now();
    try {
      await executeSqlFile(absPath, adapter);
      const elapsed = Date.now() - startedAt;
      await recordApplied(fileName, relPath, elapsed);
      logger.info(`[migration] applied: ${relPath} (${elapsed}ms)`);
      ranCount++;
    } catch (e) {
      ...
      if (readMigrationOptions(absPath).nonBlocking) {
        logger.error(`[migration] failed (continuing): ${relPath} — ${e.message}`);
      ...
      logger.error(`[migration] failed: ${relPath} — ${e.message}`);
      throw e;
    }
  }

  logger.info(`[migration] done — ${ranCount} applied, ${skipCount} skipped, ${failed.length} failed (of ${files.length}, adapter=${adapter})`);
...
async function adoptLegacySampleTables(adapter) {
  ...
  try {
    ...
    logger.warn(`[migration] sample-table check failed (skipped): ${e.message}`);
    return;
  }
  ...
  for (const base of bases) {
    ...
    if (looksLikeOurSampleTable(base, cols.get(base))) {
      ...
      try {
        ...
        logger.info(`[migration] absorbing sample table: ${base} → ${target} (data kept)`);
      } catch (e) {
        logger.warn(`[migration] could not rename the sample table ${base} → ${target}: ${e.message}`);
      }
    } else {
      logger.warn(
...
function collectSqlFilesAbs(absDirs, projectRoot) {
  ...
  if (skippedSamples.length) {
    logger.info(`[migration] sample migrations: ${skippedSamples.length} skipped (DB_SAMPLES=false)`, { kind: 'migration' });
  }
  ...
  for (const stmt of stmts) {
    ...
    try {
      ...
      if (ignoreAlreadyApplied && isAlreadyAppliedError(e)) {
        logger.info(`[migration] already-applied (ignored): ${previewSql(trimmed)} — ${e.message}`);
        continue;
      }
