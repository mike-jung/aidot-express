...
async function ensureDatabaseExists(config, logger, overrideName) {
  ...
  if (!SAFE_DB_NAME.test(name)) {
    ...
    logger.warn(`[DB] DB_DATABASE='${name}' 은 자동 생성할 수 없는 이름입니다 (영문/숫자/밑줄).`);
    return;
  }
  ...
  try {
    ...
    logger.info(`[DB] 스키마 '${name}' 을 새로 만들었습니다 (utf8mb4).`);
  } catch (e) {
    const denied = /denied|1044|1045|permission/i.test(e.message || '');
    logger.warn(`[DB] 스키마 자동 생성을 건너뜁니다 — ${e.message}`);
    if (denied) {
      // 권한이 없으면 사용자가 직접 만들어야 한다. 그 SQL 을 그대로 준다.
      logger.warn('[DB] 계정에 CREATE 권한이 없습니다. 아래를 직접 실행하세요:');
      ...
      logger.warn('[DB] 자동 생성을 끄려면 .env 에 DB_AUTO_CREATE_DATABASE=false');
    }
  } finally {
...
export async function initDb() {
  ...
  if (type === 'sqlite' || type === 'sqlite3') {
    ...
    try {
      ...
      adapter = 'sqlite';
      logger.info(`[DB] sqlite 초기화: ${file === ':memory:' ? '(in-memory)' : file}`);
    } catch (e) {
      logger.error(`[DB] sqlite 초기화 실패: ${e.message}`);
      throw e;
    }
    ...
    adapter = 'mariadb';
    logger.info(
      `[DB] mariadb 풀 초기화 ${config.db.host}:${config.db.port}/${config.db.database} ` +
      `(connectionLimit=${config.db.connectionLimit ?? 10})`,
    );
    ...
    try {
      ...
      await probe;
      logger.info('[DB] mariadb 접속 확인 OK');
      dbStatus = { available: true, adapter: 'mariadb', error: null, hint: null };
    } catch (e) {
      if (fallback) {
        logger.warn(`[DB] mariadb 초기 접속 실패: ${e.message} → sqlite 폴백`);
        ...
        pool = await openSqliteDb({ file, logger });
        adapter = 'sqlite';
        logger.info(`[DB] sqlite 폴백 초기화: ${file}`);
        dbStatus = { available: true, adapter: 'sqlite', error: null, hint: null };
      } else {
        logger.error(`[DB] mariadb 접속 실패 — ${e.message}`);
        dbStatus = {
          available: false,
          adapter: 'mariadb',
          error: e.message,
    ...
    try {
      ...
      driver = oracledb;
      adapter = 'oracle';
      logger.info(`[DB] oracle 풀 초기화 ${config.db.host}:${config.db.port}/${config.db.service}`);
    } catch (e) {
      logger.error(`[DB] oracledb 모듈 로드 실패 (npm i oracledb 필요): ${e.message} → sqlite 폴백`);
      const file = resolveSqliteFile();
      pool = await openSqliteDb({ file, logger });
      adapter = 'sqlite';
    }
    ...
    try {
      ...
      adapter = 'postgres';
      logger.info(`[DB] postgres 풀 초기화 ${config.db.host}:${config.db.port}/${config.db.database}`);
      try {
        const client = await pool.connect();
        try { await client.query('SELECT 1'); logger.info('[DB] postgres 접속 확인 OK'); }
        finally { client.release(); }
      } catch (e) {
        if (fallback) {
          logger.warn(`[DB] postgres 초기 접속 실패: ${e.message} → sqlite 폴백`);
          try { await pool.end(); } catch {}
          pool = null;
          const file = resolveSqliteFile();
          pool = await openSqliteDb({ file, logger });
          ...
          logger.error(`[DB] postgres 초기 접속 실패: ${e.message}`);
        }
      }
    } catch (e) {
      logger.error(`[DB] pg 모듈 로드 실패: ${e.message}`);
      if (fallback) {
        logger.warn('[DB] sqlite 폴백');
        const file = resolveSqliteFile();
        pool = await openSqliteDb({ file, logger });
        adapter = 'sqlite';
      }
    ...
    // 알 수 없는 type → sqlite
    logger.warn(`[DB] 알 수 없는 DB_TYPE='${type}'. sqlite 로 기본 설정.`);
    const file = resolveSqliteFile();
    pool = await openSqliteDb({ file, logger });
    adapter = 'sqlite';
  }
...
export async function closeDb() {
  ...
  try {
    ...
    logger.info(`[DB:${adapter}] 종료`);
  } catch (e) {
    logger.error(`[DB] 종료 실패: ${e.message}`);
  } finally {
    pool = null;
  }
}
