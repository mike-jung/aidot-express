...
export async function loadServicesFromDir(dir) {
  ...
  for (const file of files) {
    ...
    if (isBlocked('services', baseName)) {   // ★ v1.13.0
      ...
      logger.warn(`[Service] ⛔ ${baseName} is blocked and was not loaded${info?.reason ? ` — ${info.reason}` : ''}`);
...
export async function loadControllersFromDir(app, dir) {
  ...
  for (const file of files) {
    ...
    if (isBlocked('controllers', baseName)) {
      ...
      logger.warn(`[Controller] ⛔ ${baseName} is blocked and was not loaded${info?.reason ? ` — ${info.reason}` : ''}`);
...
export function unregisterControllerByBasePath(app, basePath) {
  ...
  logger.info(`[Controller] unregistered: ${prev.ctor?.name || '?'} (basePath=${basePath})`);
  return true;
}
