...
export async function loadSingleSqlFile(file, logger) {
  ...
  if (logger) logger.info(`[SQL] 개별 로드: ${fileName} (${res?.count ?? '?'}개 쿼리)`);
  return res;
}
...
export async function loadSqlFilesFromDir(sqlDir, logger) {
  ...
  for (const file of files) {
    ...
    if (isBlocked('sqls', rel0.replace(/\.sql$/i, ''))) {
      ...
      if (logger) logger.warn(`[SQL] ⛔ ${rel0} 은 막혀 있어 로드하지 않습니다${info?.reason ? ` — ${info.reason}` : ''}`);
      continue;
    }
    const content = fs.readFileSync(file, 'utf8');
    ...
    if (logger) {
      logger.info(`[SQL] 로딩: ${fileName} (${Object.keys(statements).length} 문)`);
    }
  }
  return registry;
}
