...
export function enqueue(rec) {
  ...
  if (queue.length > HARD_LIMIT) {
    ...
    if (dropped === cut) {   // 처음 넘쳤을 때만 알린다 (로그 폭주 방지)
      logger.warn(`[trace] 큐가 넘쳐 오래된 기록을 버립니다 — DB 쓰기가 따라오지 못하고 있습니다.`);
    }
  }
...
export async function flush() {
  ...
  try {
    ...
    if ((ins.rowsAffected ?? batch.length) < batch.length) {
      ...
      if (dup) { stats.duplicates = (stats.duplicates || 0) + dup; logger.warn(`[trace] 이미 있는 요청 ID ${dup}건 건너뜀 (X-Request-Id 재사용)`); }
    }
    ...
    if (dropped) {
      logger.warn(`[trace] 밀려서 버린 기록 ${dropped}건`);
      dropped = 0;
    }
    maybeRunRetention();
  } catch (e) {
    /* 추적 실패가 서비스를 깨뜨리면 안 된다. 버리고 다음으로 간다. */
    logger.warn(`[trace] 기록 저장 실패 (${batch.length}건 버림): ${String(e.message).slice(0, 160)}`);
  } finally {
    flushing = false;
    /* 비우는 사이에 또 쌓였으면 이어서.
...
function updateLevel() {
  ...
  if (level !== prev && level !== lastAnnounced) {
    ...
    if (level === LEVEL.NORMAL) {
      logger.info('[trace] 부하가 가셨습니다 — 전체 기록으로 돌아갑니다.');
    } else {
      const what = {
      ...
      logger.warn(`[trace] 쓰기가 밀리고 있습니다(대기 ${n}건) — ${what}. `
        + '부하가 가시면 자동으로 돌아옵니다.');
    }
  }
}
...
export async function runRetention() {
  ...
  try {
    ...
    if (result.deletedTraces > 0) {
      ...
      logger.info(`[trace] 보관 정리 — 추적 ${result.deletedTraces}건 · 단계 ${result.deletedSteps}건 삭제 (보관 ${days}일 · 최대 ${maxRecords}건)`);
    }
    stats.retention = result;
    return result;
  } catch (e) {
    logger.warn(`[trace] 보관 정리 실패(무시): ${String(e.message).slice(0, 160)}`);
