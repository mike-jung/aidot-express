  ...
  writeRaw(chunk) {
    ...
    try {
      ...
      if (!ok && this.res.writableLength > this.hub.opts.maxBufferedBytes) {
        logger.warn(`[sse] 느린 클라이언트 연결 종료 id=${this.id} buffered=${this.res.writableLength}`);
...
class SseHub {
  ...
  _remove(client, reason) {
    ...
    logger.debug(`[sse] 연결 종료 id=${client.id} channel=${client.channel} reason=${reason} sent=${client.sent}`);
  ...
  subscribe(client, channelName = 'default', opts = {}) {
    ...
    logger.info(`[sse] 구독 id=${client.id} channel=${name} ip=${client.ip} user=${client.user ?? '-'} (총 ${this.clients.size}명)`);
    ...
    if (Number.isFinite(from) && from > 0) {
      ...
      if (missed.length) logger.info(`[sse] 재전송 ${missed.length}건 (id>${from}) → ${client.id}`);
    }
  ...
  publish(channelName, data, opts = {}) {
    ...
    logger.debug(`[sse] 발행 channel=${name} event=${evt.event} id=${evt.id} → ${n}명`,
      { kind: 'internal' });
  ...
  closeAll(reason = 'server-shutdown') {
    ...
    this._stopHeartbeat();
    if (n) logger.info(`[sse] 종료로 ${n}개 스트림을 닫았습니다`);
    return n;
  }
}
