...
export default class BookService {
  ...
  // 단건 조회
  async getById(id) {
    this.log.debug(`${this.constructor.name}::getById called -> id=${id}`);
  ...
  async create(payload) {
    this.log.info(`${this.constructor.name}::create called`);
  ...
  async updateName(id, payload) {
    this.log.info(`${this.constructor.name}::updateName called -> id=${id}`);
  ...
  // 삭제 (patch-16: 추가)
  async remove(id) {
    this.log.info(`${this.constructor.name}::remove called -> id=${id}`);
