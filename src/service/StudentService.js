...
export default class StudentService {
  ...
  // 전체 학생 목록 조회
  async list() {
    this.log.info(`StudentService::list 호출됨`);
  ...
  async listPaged(opts) {
    this.log.info(`StudentService::listPaged 호출됨 -> page=${opts.page} perPage=${opts.perPage}`);
  ...
  // 학생 ID로 학생 정보 조회
  async getById(id) {
    this.log.debug(`StudentService::getById 호출됨 -> id=${id}`);
  ...
  async create(student) {
    this.log.info(`StudentService::create 호출됨 -> id=${student.id} name=${student.name}`);
  ...
  async updateName(id, name) {
    this.log.info(`StudentService::updateName 호출됨 -> id=${id} name=${name}`);
  ...
  // 학생 삭제
  async remove(id) {
    this.log.info(`StudentService::remove 호출됨 -> id=${id}`);
