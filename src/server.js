...
export async function createServer() {
  ...
  if (hasAdminDist) {
    ...
    logger.info(`[server] serving admin-client from ${adminDistDir}`);
    ...
    logger.warn(`[server] admin-client/dist not found — serving public/ at the root. ` +
  ...
  if (dbIsReplica) {
    logger.warn('[migration] this database is read-only (replica) — skipping migrations, samples and the initial account. '
      + '스키마는 복제로 따라옵니다.');
    bootStatus.migration = { ok: true, skipped: true, reason: 'replica(read_only)' };
  } else if (db.isDbUnavailable()) {
    logger.warn('[migration] no database connection — skipping migrations (connect the database and restart)');
  ...
  if (db.isDbUnavailable()) {
    logger.warn('[admin] no database — skipping the first admin account and the security check');
    ...
    logger.warn('[admin] database is read-only (replica) — skipping the initial admin account (it arrives by replication).');
    ...
    try {
      ...
      logger.warn(`[admin] skipped creating the first admin: ${e.message}`);
      bootStatus.adminSeeded = false;
    ...
    try {
      ...
      logger.warn(`[admin] default-password check failed (ignored): ${e.message}`);
    }
  ...
  if (bootProblems.length) {
    logger.warn(`[boot] ⚠ files skipped because the import failed: ${bootProblems.length} — ` + bootProblems.map((b) => `${path.relative(projectRoot, b.file).replace(/\\/g, '/')} (${b.message.split('\n')[0].slice(0, 120)})`).join(' · '));
  }
  ...
  try {
    ...
    logger.info('[admin] app injected into ControllerMetaService');
  } catch (e) {
    logger.warn(`[admin] could not load ControllerMetaService (lib/admin not in use?): ${e.message}`);
  }
  ...
  try {
    ...
    setAppForBackup(app);
    logger.info('[admin] app injected into BackupService');
  } catch (e) {
    logger.warn(`[admin] could not load BackupService: ${e.message}`);
  }
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(legacyLoaderPath)) {
        ...
        logger.info('[legacy] legacy/loader.mjs missing — skipping legacy controllers');
      }
    } catch (e) {
      logger.warn(`[legacy] error while loading: ${e.message}`);
    }

    app.use(dynamicRouter);
    logger.info('[admin] dynamicRouter registered (for dynamic controllers)');
  } catch (e) {
    logger.warn(`[admin] could not register dynamicRouter: ${e.message}`);
  }
  ...
  try {
    ...
    if (db.isDbUnavailable()) {
      logger.warn('[admin] no database — not persisting metrics (live metrics on screen come from memory)');
    } else if (container.has('MetricsService')) {
      const metricsService = container.resolve('MetricsService');
      await metricsService.start();
      logger.info('[admin] MetricsService started (monitoring is on)');
    } else {
      logger.warn('[admin] MetricsService not registered — collecting only, no storage or alerts');
    }
  } catch (e) {
    logger.warn(`[admin] could not start monitoring (the service keeps running): ${e.message}`);
  }
  ...
  try {
    if (container.has('AccessLogService')) {
      ...
      logger.warn('[admin] AccessLogService not registered — access stats are off');
    }
  } catch (e) {
    logger.warn(`[admin] could not start AccessLogService: ${e.message}`);
  }
  ...
  try {
    ...
    await initHa({
      ...
      onRoleChange: (ev) => {
        logger.warn(`[ha] role ${ev.from} → ${ev.to} (${ev.reason}) token=${ev.fenceToken}`);
        ...
        try {
          ...
          child.on('error', (e) => logger.error(`[ha-hook] failed: ${e.message}`));
        } catch (e) {
          logger.error(`[ha-hook] failed: ${e.message}`);
        }
      },
    });
  } catch (e) {
  ...
  if (hasAdminDist) {
    ...
    logger.info('[server] SPA history fallback on (admin-client/dist/index.html)');
  }
