...
function readEnvFileDecoded(filePath) {
  ...
  if (buf.length >= 2 && buf[0] === 0xFF && buf[1] === 0xFE) {
    ...
    try {
      ...
      console.log(`[config] ⚙ ${filePath} 를 UTF-16LE → UTF-8 로 자동 변환했습니다.`);
    } catch (e) {
      console.warn(`[config] UTF-8 자동 변환 실패 (무시됨): ${e.message}`);
    }
    return text;
  }
...
(function loadEnvFiles() {
  ...
  for (const p of loadOrder) {
    ...
    try {
      ...
      console.error(`[config] .env 로드 실패: ${p}`);
  ...
  if (loadedEnvFiles.length === 0) {
    lines.push('[config] ⚠ .env 파일을 찾지 못했습니다. 기본값 + 환경변수로만 기동합니다.');
    lines.push('[config]   검색한 위치 (우선순위 순):');
    for (const p of uniq) lines.push(`[config]     - ${p}`);
  } else {
    lines.push(`[config] .env 로드 완료 (${loadedEnvFiles.length}개, 우선순위 순):`);
  ...
  lines.push(`[config] process.env 주요 값: ${envStatus.join(', ')}`);
