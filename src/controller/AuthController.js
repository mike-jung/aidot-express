...
export default class AuthController {
  ...
  async signup(params, _req, res) {
    this.log.info(`AuthController::signup 호출됨 -> params=${JSON.stringify({ ...params, password: '***' })}`);
  ...
  async login(params, req, res) {
    this.log.info(`AuthController::login 호출됨 -> username=${params.username}`);
  ...
  async refresh(params, req, res) {
    this.log.info(`AuthController::refresh 호출됨`);
  ...
  async logout(params, req, res) {
    this.log.info(`AuthController::logout 호출됨`);
  ...
  async logoutAll(params, req, res) {
    this.log.info(`AuthController::logoutAll 호출됨 -> userId=${req.user?.id}`);
  ...
  @Auth()
  async me(params, req) {
    this.log.info(`AuthController::me 호출됨 -> userId=${req.user?.id}`);
