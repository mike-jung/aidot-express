...
export class Watchdog {
  ...
  start() {
    if (!this.cfg.enabled) {
      this.logger.info('[watchdog] 생존 감시 꺼짐 (SUPERVISOR_WATCHDOG=false)');
      return;
    }
    this.stop();
    this._timer = setInterval(() => { this._tick().catch(() => {}); }, Math.max(1_000, this.cfg.intervalMs));
    this._timer.unref?.();
    this.logger.info(`[watchdog] 생존 감시 시작 — ${Math.round(this.cfg.intervalMs / 1000)}초마다 /health/live, ` +
      `연속 ${this.cfg.failures}회 무응답이면 자동 재기동 (시간당 최대 ${this.cfg.maxRestartsPerHour || '∞'}회)`);
  }
    ...
    try {
      ...
      if (r.ok) {
        if (this.consecutiveFailures > 0) {
          this.logger.info(`[watchdog] 메인 서버가 다시 응답합니다 (${this.consecutiveFailures}회 실패 뒤)`);
      ...
      this.logger.warn(`[watchdog] 메인 서버 무응답 ${this.consecutiveFailures}/${this.cfg.failures} (${r.kind}: ${r.detail})` +
      ...
      if (d.action === 'none' && d.reason === 'too-soon' && this.consecutiveFailures === this.cfg.failures) {
        ...
        this.logger.warn(`[watchdog] 재기동 조건은 찼지만 직전 재기동 뒤 ${Math.round(this.cfg.minRestartIntervalMs / 1000)}초가 지나지 않았습니다 — ${wait}초 더 기다립니다 (기동 직후 죽는 경우의 무한 반복 방지)`);
      }
  ...
  async _restart(reason, detail) {
    ...
    this.logger.error(`[watchdog] ★ 자동 재기동 #${no} — 이유: ${reason} — ${detail}` +
    ...
    try {
      ...
      if (reason === 'unresponsive' && this.cfg.hangDump && this.sup.child && this.sup.child.exitCode === null && this.sup.inspectPort) {
        ...
        if (dump.ok) {
          this.logger.error(`[watchdog] 멈춘 지점의 JS 스택 (${dump.elapsedMs}ms 만에 확보${dump.reportWritten ? `, 보고서 ${path.basename(reportFile)}` : ''}):`);
          for (const line of dump.stack) this.logger.error(`[watchdog]     at ${line}`);
        } else {
          this.logger.warn(`[watchdog] 스택을 꺼내지 못했습니다 — ${dump.error} (프로세스가 완전히 얼어붙었거나 인스펙터를 켤 수 없는 상태)`);
        }
      }
      ...
      this.logger.warn(`[watchdog] 자동 재기동 #${no} ${ok ? '완료' : '시도했지만 아직 응답 없음'} — new pid=${st.pid} (${Date.now() - startedAt}ms)`);
    } catch (e) {
      this.lastRestartAt = Date.now();
      this.totalAutoRestarts = no;
      ...
      this.logger.error(`[watchdog] 자동 재기동 #${no} 실패: ${e.message}`);
    } finally {
      this.restarting = false;
    }
  }
  ...
  _halt(why) {
    ...
    this.logger.error(`[watchdog] ✖ 자동 재기동을 멈춥니다 — 한 시간에 ${this.cfg.maxRestartsPerHour}회를 넘었습니다 (${why}). ` +
