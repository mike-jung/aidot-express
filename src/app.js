...
async function main() {
  ...
  try {
    ...
    if (r.moved > 0 || r.skipped > 0) {
      logger.info(`[meta] migrated legacy meta files: moved=${r.moved}, skipped=${r.skipped}`);
    }
    if (r.errors.length) {
      logger.warn(`[meta] during migration: ${r.errors.length} error: ${r.errors.slice(0,3).join(' ; ')}`);
    }
  } catch (e) {
    logger.warn(`[meta] migration failed (ignored): ${e.message}`);
  }
  ...
  try {
    ...
    logger.error(`[secure] column encryption failed to start: ${e.message}`);
  ...
  const shutdown = (signal, exitCode = 0) => {
    ...
    try { sseHub.closeAll(signal); } catch (e) { logger.warn(`[sse] stop handled failed: ${e.message}`); }
    ...
    server.close(async () => {
      try {
        ...
        if (accessSvc) {
          ...
          logger.info('[admin] AccessLogService queue flushed');
        }
      } catch (e) {
        logger.warn(`[admin] AccessLogService stop failed: ${e.message}`);
      }
  ...
  if (process.env.AIDOT_SUPERVISED === '1' && process.platform !== 'win32') {
    ...
    const watchdog = setInterval(() => {
      if (process.ppid !== parentPid) {
        logger.warn('[watchdog] the supervisor process is gone — stopping the main server too.');
