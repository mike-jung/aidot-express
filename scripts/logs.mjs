...
if (!files.length) {
  console.log(`로그 파일이 없습니다: ${dir}`);
  console.log('서버를 한 번 띄우면 만들어집니다 — npm start');
  process.exit(0);
}
...
if (q.fields) {
  ...
  console.log(`\n  로그 ${lines.length.toLocaleString()}줄 · ${files.length}개 파일 · ${dir}\n`);
  console.log('  kind   (유형)');
  for (const [k, n] of count((l) => l.kind)) console.log(`    ${k.padEnd(10)} ${String(n).padStart(7)}줄    npm run logs ${k}`);
  console.log('\n  level  (수준)');
  for (const [k, n] of count((l) => l.level)) console.log(`    ${k.padEnd(10)} ${String(n).padStart(7)}줄    npm run logs ${k.toLowerCase()}`);
  const reqs = count((l) => l.requestId);
  console.log(`\n  요청 ID  ${reqs.length}개 — 예: npm run logs -r ${reqs[0]?.[0] || 'a3f9c210'}`);
  console.log('\n  시간     npm run logs --around 14:30 --window 5s   ← 그 시각 앞뒤 전부\n');
  process.exit(0);
}
...
if (q.around) {
  ...
  if (Number.isNaN(t)) { console.error(`시각을 알아볼 수 없습니다: ${q.around}`); process.exit(1); }
...
console.log(`\n  ${why.length ? why.join(' · ') : '전체'} — ${sel.length.toLocaleString()}줄`
...
if (!shown.length) {
  console.log('  조건에 맞는 줄이 없습니다.');
  console.log('  무엇으로 거를 수 있는지 보려면:  npm run logs -- --fields\n');
}
...
function usage() {
  console.log(`
  로그 보기 — 외울 문법이 없습니다. 낱말을 나열하세요.

    npm run logs sql                 SQL 만
    npm run logs errors today        오늘 오류만
    npm run logs slow                500ms 넘는 것
    npm run logs sql slow            느린 쿼리
    npm run logs a3f9c210            그 요청에서 일어난 전부

  ★ 시간으로 (파일을 쪼갰다면 불가능한 것)

    npm run logs -- --around 14:30 --window 5s     그 시각 앞뒤 5초, 유형 무관

  거를 수 있는 것 보기

    npm run logs -- --fields         지금 로그에 실제로 있는 값들

  낱말은 순서 무관, 복수형·대소문자·한글 모두 받습니다.
    sql=queries=db · errors=오류 · today=오늘 · slow=느린

  옵션
    -r <요청ID>          그 요청만          -g <검색어>     본문 검색
    --slower-than <ms>   느린 것만          -n <개수>       표시 줄 수 (기본 200)
    --since 1h|1d        최근 기간          --fields        거를 수 있는 값
`);
}
