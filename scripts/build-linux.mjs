...
function buildWsl() {
  ...
  console.log(`▶ Building inside WSL (${distro})`);
...
try {
  if (want === '--native' || (!want && !isWin)) { buildNative(); }
    ...
    /* Windows 에서 자동 선택 */
    console.log('· AppImage cannot be built on Windows (a tool limitation).');
    if (has('wsl.exe', ['-l', '-q'])) { buildWsl(); }
    else if (has('docker')) { buildDocker(); }
    else {
      console.log('\n· Neither WSL nor Docker found — building tar.gz instead.');
      console.log('  If you need AppImage, install one of these:');
      console.log('    WSL2   : wsl --install -d Ubuntu   (recommended — fast)');
  ...
  console.error('  You can try another method:');
  console.error('    npm run dist:linux -- --wsl');
  console.error('    npm run dist:linux -- --docker');
  console.error('    npm run dist:linux -- --targz    (no AppImage)');
  process.exit(1);
}
