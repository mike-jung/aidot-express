...
function step(label, cmdArgs, { optional = false } = {}) {
  ...
  if (!ok && !optional) {
    ...
    report();
    console.error(`\n✗ '${label}' 에서 멈췄습니다. 위 오류를 먼저 해결하세요.`);
    console.error('  앞 단계 결과물은 그대로 남아 있습니다.');
    process.exit(1);
  }
  if (!ok) console.log(`\n· '${label}' 은 실패했지만 넘어갑니다 (선택 단계).`);
  return ok;
}
...
function report() {
  ...
  for (const r of results) {
    ...
    console.log(`  ${mark} ${r.label.padEnd(34)} ${r.secs.padStart(4)}초`);
  }
  console.log(`  ${'전체'.padEnd(36)} ${((Date.now() - started) / 1000).toFixed(0).padStart(4)}초`);
...
report();
if (dryRun) console.log('\n--dry-run 이었습니다 — 올리지는 않았습니다.');
else if (!noPublish) console.log('\n다음: GitHub → Releases 에서 초안을 확인하고 Publish 하세요.');
