...
try {
  ...
  try { pw = require('/home/claude/.npm-global/lib/node_modules/playwright'); }
    ...
    console.log('smoke: playwright missing — skipping (npm i -D playwright to use it)');
    process.exit(0);
  }
}
...
const SCREENS = [
  ...
  ['로그', '/logs'], ['request trace', '/trace'], ['OpenAPI 스펙', '/openapi'],
  ['backup and restore', '/backup', true],
  ['column encryption', '/secure-columns', true],
...
try {
  ...
  console.error('smoke: sign-in failed —', String(e.message).slice(0, 120));
  await browser.close();
  process.exit(1);
}
...
for (const [name, href, optional] of SCREENS) {
  ...
  if (!(await link.count())) {
    if (optional) {
      ...
      hidden.push(name);
      console.log(`  · ${name} — off in settings, skipped`);
    } else {
      bad.push({ name, len: 0, errs: ['메뉴에 없음 — 사라졌거나 라우트가 깨졌습니다'] });
      console.log(`  ✗ ${name} — missing from the menu (should not be hidden)`);
    }
    continue;
  }
  await link.click();
  ...
  if (newErrs.length || len < MIN_BODY) {
    ...
    console.log(`  ✗ ${name} — body ${len} chars${newErrs.length ? `, 오류 ${newErrs.length}건` : ' (내용이 거의 없다)'}`);
    for (const e of newErrs.slice(0, 2)) console.log(`      ${e}`);
  } else {
    console.log(`  ✓ ${name} — body ${len} chars`);
  }
}

await browser.close();
console.log(`\nsmoke: ${checked - bad.length} / ${checked} OK`
  + (hidden.length ? ` · 설정으로 꺼진 화면 ${hidden.length}개 (${hidden.join(', ')})` : ''));
if (bad.length) { console.error(`${bad.length} screens failed to open — see the errors above.`); process.exit(1); }
