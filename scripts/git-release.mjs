...
if (!token || !repo) {
  console.error(`✗ .env 에 GITHUB_TOKEN 과 ${toFull ? 'GITHUB_REPO' : 'PUBLIC_REPO'} 가 필요합니다.`);
  process.exit(1);
}
if (!toFull && !env.PUBLIC_REPO) {
  console.log('⚠ PUBLIC_REPO 가 없어 GITHUB_REPO 로 올립니다.');
  console.log('  릴리스는 보통 공개 저장소로 갑니다 — .env 에 PUBLIC_REPO 를 넣어 주세요.');
}
console.log(`릴리스 대상: ${repo}${toFull ? '  (--to-full · 사내 배포)' : '  (공개 저장소)'}`);
...
if (!fs.existsSync(distDir)) {
  console.error(`✗ ${distDir} 가 없습니다.`);
  console.error('  먼저 빌드하세요:  npm run dist:win   (또는 dist:linux · dist:all)');
  process.exit(1);
}
...
if (stale.length) {
  console.log(`\n· 다른 버전 ${stale.length}개는 건너뜁니다 (이번은 v${version}):`);
  for (const f of stale.slice(0, 6)) console.log(`    ${f}`);
  if (stale.length > 6) console.log(`    … 외 ${stale.length - 6}개`);
  console.log('  깨끗하게 빌드하려면 dist-electron 폴더를 지우고 다시 만드세요.');
}

if (fullOnes.length && !allowFull) {
  console.log(`\n⚠ full 판 ${fullOnes.length}개는 올리지 않습니다 (공개 릴리스이므로):`);
  for (const f of fullOnes) console.log(`    ${f}`);
  console.log('  full 판을 전달해야 한다면 파일을 직접 건네세요.');
}
if (fullOnes.length && allowFull) {
  console.log('\n⚠⚠ --allow-full 이 켜져 있습니다. 아래는 **엔터프라이즈 기능이 들어간** 설치 파일입니다:');
  for (const f of fullOnes) console.log(`    ${f}`);
  console.log('  공개 저장소에 올리려는 것이 맞습니까? 아니라면 지금 Ctrl+C 를 누르세요. (5초)');
  await new Promise((r) => setTimeout(r, 5000));
}
if (!assets.length) {
  console.error(`✗ ${distDir} 에 올릴 설치 파일이 없습니다.`);
  console.error('  그 폴더에 있는 것:', fs.readdirSync(distDir).slice(0, 10).join(', ') || '(비어 있음)');
  console.error('  npm run dist:win 이 끝까지 성공했는지 확인하세요.');
  process.exit(1);
}

console.log(`릴리스 ${tag} — 파일 ${assets.length}개`);
...
if (notes) console.log(`설명은 ${notesFile} 의 "${notes.title}" 절을 씁니다.`);
else console.log('⚠ RELEASE_NOTES.md 에 이번 판이 없어 기본 설명을 씁니다 — 한 절 적어 주세요.');
...
for (const file of assets) {
  ...
  process.stdout.write(`  올리는 중 ${name} … `);
...
console.log(`\n끝났습니다 → https://github.com/${repo}/releases`);
console.log('  아직 **초안(draft)** 입니다. 내용을 확인하고 [Publish release] 를 누르면 공개됩니다.');
console.log('  설명·제목·파일은 그 화면에서 언제든 고칠 수 있습니다.');
