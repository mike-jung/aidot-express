...
if (problems.length) {
  console.error('같은 이름의 SQL 이 여러 번 정의돼 있습니다 — 뒤엣것이 앞엣것을 조용히 덮어씁니다:');
  for (const p of problems) console.error(`  ✗ ${p.file}  →  @name: ${p.name} (${p.count}번)`);
  process.exit(1);
}
console.log(`sql-names: ${files.length}개 파일 — 중복 정의 없음`);
