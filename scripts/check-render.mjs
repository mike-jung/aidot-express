...
if (!sfcPath) { console.error('@vue/compiler-sfc 없음 — admin-client 의존성을 설치하세요.'); process.exit(2); }
...
if (!fs.existsSync(vueBrowser)) { console.error('vue 브라우저 빌드 없음'); process.exit(2); }
...
if (!playwright) {
  console.log('playwright 없음 — 브라우저 렌더 검증을 건너뜁니다 (문법 검증은 check-sfc.mjs 로).');
  process.exit(0);
}
...
for (const file of targets) {
  if (!fs.existsSync(file)) { console.log(`  건너뜀(없음): ${file}`); continue; }
...
for (const r of results) {
  ...
  if (!okMount) bad++;
  console.log(`  ${okMount ? 'PASS' : 'FAIL'}  ${r.id.padEnd(22)} 마운트=${r.mounted ? 'O' : 'X'} 노드=${String(r.nodes).padStart(4)} 텍스트=${String(r.chars).padStart(4)}자`);
  for (const p of r.problems.slice(0, 4)) console.log(`         ↳ ${p}`);
}
console.log(`\n대상 ${compiled.length}개 — 통과 ${results.length - bad - skipped}, 실패 ${bad}, 건너뜀 ${skipped}`);
