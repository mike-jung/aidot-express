...
async function main() {
  ...
  if (!fs.existsSync(dir)) {
    ...
    if (mod) {
      ...
      if (fake) {
        say('✖', '드라이버', `가짜입니다 — ${fake}`);
        console.log(`     A test stub is still installed. Remove it and reinstall:`);
    ...
    try {
      ...
      if (config.db.samples !== false && config.db.sampleSchemaSeparate !== false) {
        ...
        try {
          ...
          console.log(`     Ask your DBA to run:  GRANT ALL ON \`${schema}\`.* TO '${user}'@'%';`);
  ...
  if (problems.length) {
    console.log(`\n  ✖ ${problems.length} problems — fix them as described above and run again.\n`);
    process.exit(1);
  }
...
main().catch((e) => {
  console.error('\n  ✖ Error during the check:', e.message, '\n');
  process.exit(1);
});
