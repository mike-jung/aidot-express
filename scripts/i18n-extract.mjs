...
if (!file) { console.error('Usage: node scripts/i18n-extract.mjs <file.vue> [--apply]'); process.exit(2); }
...
if (!tplMatch) { console.error('template not found'); process.exit(2); }
let tpl = tplMatch[1];
...
if (!items.length) { console.log('No Korean to replace.'); process.exit(0); }
...
console.log(`${path.relative(ROOT, abs)} — ${items.length}`);
for (const it of items) console.log(`  ${ns}.${it.key}  ${it.ko}`);

if (!APPLY) { console.log('\n(preview only — pass --apply to write)'); process.exit(0); }
...
for (const [p, block] of [[koPath, koBlock], [enPath, enBlock]]) {
  ...
  if (new RegExp(`\\n  ${ns}: \\{`).test(s)) {
    ...
    console.error(`\n⚠⚠ ${ns} block already exists — could not add to the dictionary.`);
    console.error(`   t('${ns}.kN') went into the screen, but with no dictionary value **the key shows as-is.**`);
    console.error(`   Merge the list below into the ${ns} block of ${path.basename(p)} by hand,`);
    console.error(`   or use **a name that does not clash** (e.g. ${ns}Msg).`);
    console.error(`   ⚠ ${ns}2 with a number appended may already be in use — check first.\n`);
    continue;
  }
  s = s.replace(/^export default \{\n/m, (m) => m + block);
  fs.writeFileSync(p, s, 'utf8');
}
console.log(`\nApplied. Replace the "TODO:" entries in en.js with English (${items.length}).`);
