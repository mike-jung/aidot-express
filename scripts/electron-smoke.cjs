...
(async () => {
  ...
  console.log(`\nelectron-smoke result: pass=${pass} fail=${failures.length}` + (failures.length ? `\n실패: ${failures.join(' | ')}` : ''));
  process.exit(failures.length ? 1 : 0);
})().catch((e) => {
  console.error(`\nelectron-smoke aborted: ${e.message}`);
  console.log(`checks completed: pass=${pass} fail=${failures.length}`);
  process.exit(1);
});
