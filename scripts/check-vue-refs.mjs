...
if (uniq.length) {
  console.error('Used without importing — opening that screen throws ReferenceError:');
  for (const p of uniq) console.error(`  ✗ ${p.file}  →  ${p.name}() called, no import`);
  console.error(`vue-refs: ok=${files.length - uniq.length} fail=${uniq.length}`);
  process.exit(1);
}
console.log(`vue-refs: ${files.length} files checked — no problems`);
