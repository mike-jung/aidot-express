...
if (env.GITHUB_PASSWORD || env.GITHUB_PASSWD) {
  console.error('⚠ GITHUB_PASSWORD 는 쓰지 않습니다. GitHub 가 2021년부터 비밀번호 push 를 막았습니다.');
  console.error('  GITHUB_TOKEN 에 Personal Access Token 을 넣어 주세요.');
}
if (!token) { console.error('✗ .env 에 GITHUB_TOKEN 이 없습니다. (위 주석 참고)'); process.exit(1); }
if (!repo || !repo.includes('/')) { console.error('✗ .env 의 GITHUB_REPO 를 "계정/저장소" 형태로 적어 주세요.'); process.exit(1); }

/* ── git 저장소 준비 ────────────────────────────────────────────────── */
if (!fs.existsSync(path.join(ROOT, '.git'))) {
  console.log('git 저장소가 없어 새로 만듭니다.');
  sh('git', ['init']);
...
function gitPush(args) {
  ...
  try {
    ...
    console.error(`\n✗ push 실패 (exit ${e.status ?? '?'})`);
    console.error('  · "Invalid username or token" 이면 Windows 자격 증명에 옛 토큰이 남아 있을 수 있습니다.');
    console.error('    제어판 → 자격 증명 관리자 → Windows 자격 증명 → git:https://github.com 삭제');
    return false;
  }
}
...
if (!staged) {
  ...
  if (n === 0) { console.log('바뀐 것이 없고, 올릴 커밋도 없습니다.'); process.exit(0); }
  const head = quiet('git', ['log', '--oneline', '-1']) || '';
  console.log(n < 0
    ? `\n바뀐 파일은 없습니다. 이 저장소에 아직 아무것도 올린 적이 없어 커밋 전체를 올립니다.\n  최신 커밋: ${head}`
    : `\n바뀐 파일은 없지만 올리지 않은 커밋이 ${n}개 있습니다 — 그것만 올립니다.\n  최신 커밋: ${head}`);
  if (dryRun) { console.log('--dry-run 이라 여기서 멈춥니다.'); process.exit(0); }
  if (!gitPush([])) {
    console.error('  토큰 권한(Contents: Read and write)과 저장소 이름을 확인하세요.');
    process.exit(1);
  }
  console.log(`\n올렸습니다 → https://github.com/${repo}/tree/${branch}`);
  process.exit(0);
}

const lines = staged.split('\n');
console.log(`\n올라갈 파일 ${lines.length}개:`);
for (const l of lines.slice(0, 30)) console.log('  ', l);
if (lines.length > 30) console.log(`   … 그리고 ${lines.length - 30}개 더`);
...
if (lines.some(isSecretEnv)) {
  console.error('\n✗ .env 가 커밋에 들어 있습니다. .gitignore 에 추가한 뒤 다시 실행하세요.');
  sh('git', ['reset']);
  process.exit(1);
}

if (dryRun) { console.log('\n--dry-run 이라 여기서 멈춥니다.'); sh('git', ['reset']); process.exit(0); }
