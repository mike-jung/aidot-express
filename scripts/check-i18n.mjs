...
if (!codes.includes('en')) {
  console.error('  en.js 가 없습니다 — 영어가 폴백 기준입니다.');
  process.exit(1);
}
...
console.log(`\n  사전  ${codes.join(' · ')}  ·  키 ${base.length}개`);
if (problems.length) for (const p of problems) console.log(`  FAIL  ${p}`);
else console.log('  OK    모든 언어의 키가 일치합니다');
...
function usedKeys(text) {
  ...
  const body = text
    ...
    .replace(/<!--[\s\S]*?-->/g, '')
    .replace(/(^|[^:"'`\\])\/\/[^\n]*/g, '$1');
  return [...body.matchAll(/\bt\(\s*'([\w.]+)'/g)].map((m) => m[1]);
}
...
console.log(`\n  화면 문구  하드코딩 ${totalKo}줄 · 바인딩/인라인 ${totalBinding}곳 남음`);
console.log(`             완료 ${done}  ·  진행 중 ${partial}  ·  미착수 ${todo}  (파일 기준)`);
...
if (rows.some((r) => r.ko > 0 || r.bind > 0)) {
  ...
  for (const r of rows.filter((x) => x.ko > 0 || x.bind > 0).slice(0, 10)) {
    console.log(`    ${String(r.ko).padStart(4)}줄${r.bind ? ` +${String(r.bind).padStart(3)}곳` : '     '}  ${r.t ? `(t() ${r.t}회) ` : ''}${r.f}`);
  }
}
console.log('');

if (bindingBugs.length) {
  console.log('  바인딩 표현식 안에 {{ }} 가 있습니다 — 파싱이 깨집니다');
