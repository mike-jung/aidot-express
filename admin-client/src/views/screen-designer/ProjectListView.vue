...
async function onDelete(row) {
  if (!await confirmDelete(row.name, { title: t('designer.deleteTitle'), detail: t('designer.deleteDetail'), requireText: row.name })) return;
  ...
  try {
    ...
    notifyError(t('designer.deleteFailed'), e);
  } finally {
  ...
  <div class="container-fluid py-3">
    ...
    <div class="d-flex justify-content-between align-items-start mb-3">
      ...
      <div class="d-flex align-items-center gap-2">
        ...
        <button class="btn btn-outline-secondary" @click="load" :disabled="loading" :title="t('designer.refreshList')">
          ...
          <span class="ms-1">{{ loading ? t('designer.loading') : t('designer.refresh') }}</span>
        </button>
    ...
    <div v-else-if="!loading && rows.length === 0" class="card">
      <div class="card-body text-center py-5 text-secondary">
        ...
        <div class="small mb-3">{{ t('designer.emptyHint').replace('{btn}', t('designer.newProject')) }}</div>
