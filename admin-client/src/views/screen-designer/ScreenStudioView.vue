...
async function loadProject() {
  ...
  if (!currentScreen.value && activeProject.value) {
    notifyError(t('designer.notFound'), t('designer.notFoundDetail'));
...
const actions = {
  ...
  async removeRow(rowId) {
    if (!await confirmDialog({ title: t('designer.deleteRow'), message: '이 row 와 그 안의 모든 widget 을 삭제할까요?', confirmText: '삭제', variant: 'danger' })) return;
  ...
  <div class="container-fluid py-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="d-flex align-items-center gap-2">
        <button class="btn btn-sm btn-outline-secondary" @click="goBack" :title="t('designer.backToProject2')">
      ...
      <div class="save-indicator small">
        <span v-if="saveState === 'pending'" class="text-secondary">
          <i class="bi bi-circle"></i> {{ t('designer.editing') }}
        </span>
        <span v-else-if="saveState === 'saving' || saving" class="text-primary">
          <span class="spinner-border spinner-border-sm me-1"></span>{{ t('designer.saving') }}
        </span>
        <span v-else-if="saveState === 'saved'" class="text-success">
          <i class="bi bi-check-circle-fill me-1"></i>{{ t('designer.saved') }}
        </span>
        <span v-else-if="saveState === 'error'" class="text-danger">
          <i class="bi bi-exclamation-triangle me-1"></i>{{ t('designer.saveFailed') }}
        </span>
      </div>
    </div>
    ...
    <div v-if="localScreen && studioMode === 'edit'" class="card mb-3">
      <div class="card-header py-2">
        <h6 class="mb-0">
          <i class="bi bi-card-heading me-1"></i>{{ t('designer.titleArea') }}
        </h6>
      </div>
      <div class="card-body">
        <label class="form-label small">{{ t('designer.fieldType') }}</label>
        ...
        <div v-if="localScreen.header.kind !== 'none'" class="row g-3">
          <div class="col-md-6">
            <label class="form-label small">{{ t('designer.fieldTitle') }}</label>
    ...
    <ul v-if="localScreen" class="nav nav-pills mb-3">
      <li class="nav-item">
        <button class="nav-link"
          ...
          <i class="bi bi-pencil-square me-1"></i>{{ t('designer.edit') }}
        </button>
      </li>
      <li class="nav-item">
        <button class="nav-link"
          ...
          <i class="bi bi-eye me-1"></i>{{ t('designer.previewTab') }}
        </button>
      </li>
    </ul>
    ...
    <template v-if="localScreen && studioMode === 'edit'">
      ...
      <div class="studio-canvas-wrap">
        <div class="card">
          <div class="card-header py-2">
            <h6 class="mb-0">
              <i class="bi bi-layout-three-columns me-1"></i>{{ t('designer.screenLayout') }}
            </h6>
          </div>
          <div class="card-body">
