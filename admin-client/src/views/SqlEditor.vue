...
const title = computed(() => isEdit.value ? 'SQL 파일 수정' : '신규 SQL 파일');
...
async function runTest(gi) {
  ...
  try {
    ...
    if (data && data.success === false) {
      q.testError = data.error || 'SQL 실행 실패';
    ...
    if (status === 404) {
      q.testError = 'API 엔드포인트 없음 (404). 서버의 SqlMetaController.js 최신 여부 확인 + 서버 재시작 필요.';
    } else {
...
async function queryTableColumns(gi, overrideTable) {
  ...
  if (!tbl) { q._colQueryError = 'SQL에서 FROM 테이블을 찾을 수 없습니다'; return; }
