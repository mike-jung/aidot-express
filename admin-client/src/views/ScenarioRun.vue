...
async function load() {
  ...
  if (!sc) { error.value = '시나리오를 찾을 수 없습니다.'; return; }
  scenario.value = sc;
  resetResults();
}
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex gap-2">
        ...
        <button class="btn btn-sm btn-success" @click="run" :disabled="running || !scenario">
          ...
          <i v-else class="bi bi-play-fill me-1"></i>실행
        </button>
      </div>
    </div>
    ...
    <div class="card-body">
      ...
      <div v-if="scenario">
        ...
        <label class="form-check small mb-3">
          ...
          현재 어드민 토큰으로 Authorization 자동 추가
        </label>

        <!-- 진행 요약 -->
        <div v-if="stepStatus.some(s => s !== 'pending')" class="mb-3 small">
          진행: <strong>{{ overallSummary.done }}/{{ overallSummary.total }}</strong>
