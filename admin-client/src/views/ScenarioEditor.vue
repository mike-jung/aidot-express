...
async function loadCatalog() {
  try {
    ...
    error.value = '라우트 카탈로그 로드 실패: ' + (e.response?.data?.message || e.message);
    allRoutes.value = [];
  }
}
...
const ASSERT_OPS = [['exists', '있음'], ['notEmpty', '비어 있지 않음'], ['empty', '비어 있음'], ['eq', '='], ['ne', '≠'], ['contains', '포함'], ['gt', '>'], ['lt', '<'], ['matches', '정규식']];
...
async function onSave() {
  error.value = null;
  if (!scenario.name.trim()) { error.value = '이름이 필요합니다'; return; }
  if (scenario.steps.length === 0) { error.value = '단계가 1개 이상 필요합니다'; return; }
  for (const [i, s] of scenario.steps.entries()) {
    if (!s.fullPath) { error.value = `${i+1}번째 단계에 경로가 비어있습니다.`; return; }
  }
  saving.value = true;
  try {
    // list → object 변환
