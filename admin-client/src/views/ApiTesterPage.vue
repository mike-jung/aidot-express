...
function onLabelEdit(item) {
  const newLabel = prompt('라벨(이름) — 비우면 제거:', item.label || '');
...
async function onDeleteHistory(item) {
  if (!await confirmDialog({ title: '기록 삭제', message: '이 호출 기록을 삭제할까요?', detail: `${item.method || ''} ${item.path || item.url || ''}`.trim(), confirmText: '삭제', variant: 'danger' })) return;
  deleteHistoryItem(item.id);
  refreshHistory();
}

async function onClearHistory() {
  if (!await confirmDialog({ title: t('apiTester.k18'), message: t('apiTester.k19'), detail: '즐겨찾기는 보존됩니다.', confirmText: t('apiTester.k8'), variant: 'danger' })) return;
  ...
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
      <span>
        ...
        <small v-if="tab === 'routes'" class="text-secondary ms-2">{{ totalRoutes }} / {{ controllers.reduce((s,c) => s + c.routes.length, 0) }} 라우트</small>
        <small v-else-if="tab === 'history'" class="text-secondary ms-2">{{ history.length }} 호출</small>
        <small v-else class="text-secondary ms-2">{{ history.filter(h => h.isFavorite).length }} 즐겨찾기</small>
      </span>
        ...
        <div class="col-lg-5">
          ...
          <div v-if="tab === 'routes'" class="mt-2">
            ...
            <div class="text-center small text-secondary mt-1">
              컨트롤러 {{ filteredControllers.length ? ((routePage - 1) * ROUTES_PER_PAGE) + 1 : 0 }}-{{ Math.min(routePage * ROUTES_PER_PAGE, filteredControllers.length) }} / {{ filteredControllers.length }}
            </div>
          </div>
