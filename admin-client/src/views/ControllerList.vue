...
async function onToggleBlock(r) {
  if (isBlocked(r.name)) {
    if (!window.confirm(t('designer.ctl_enableAsk').replace('{name}', r.name))) return;
    await unblock(r.name);
    await load();
    return;
  }
  const reason = window.prompt(
    t('designer.ctl_disableHead').replace('{name}', r.name) +
    t('designer.ctl_disable1') +
    t('designer.ctl_disable2') +
    t('designer.ctl_disable3') +
    t('designer.ctl_reasonAsk'), t('designer.ctl_reasonEx'));
