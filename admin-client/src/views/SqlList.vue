...
const loadState = useLoadState('sql');
const perPage = ref(10);
const rows = ref([]);
const { onlyWorkspace, originParam } = useOnlyWorkspace();   // ★ v1.11.7 [내 것만] 스위치
/* ★ v1.13.0 — 막기/풀기 (컨트롤러 목록과 같은 방식) */
const { refresh: refreshBlocked, isBlocked, infoOf, block, unblock } = useBlocklist('sqls');
async function onToggleBlock(r) {
  const nm = r.name;
  if (isBlocked(nm)) {
    if (!window.confirm(t('designer.ctl_enableShort').replace('{name}', nm))) return;
    await unblock(nm); await load(); return;
  }
  const reason = window.prompt(
    t('designer.ctl_disableHead').replace('{name}', nm) +
    t('designer.ctl_disable2') +
    t('designer.ctl_disable3') +
    t('designer.sql_disable3') +
    `사유를 적어 주세요:`, t('designer.ctl_reasonEx'));
...
async function load() {
  ...
  error.value = null;
  try {
    const r = await http.get('/api/admin/sqls/paged', {
...
async function onDelete(row) {
  if (!await confirmDelete(`${row.name}.sql`, { title: 'SQL 파일 삭제', detail: '파일과 SqlRegistry 에서 모두 제거됩니다.', requireText: row.name })) return;
  try {
    await http.delete(`/api/admin/sqls/${row.id}`);
    await load();
  } catch (e) {
    notifyError('요청 실패', e);
  }
...
<template>
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
      <span>
        <i class="bi bi-database me-2"></i>{{ t('sqlList.title') }}
      ...
      <div class="d-flex gap-2 align-items-center">
        ...
        <button class="btn btn-sm btn-primary" @click="openCreate">
          <i class="bi bi-plus-lg me-1"></i>{{ t('sqlList.newSql') }}
        </button>
      </div>
    </div>
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table table-hover align-middle">
          <thead class="text-secondary small">
            <tr>
              <th>{{ t('sqlList.colFile') }}</th>
              <th>{{ t('sqlList.colTable') }}</th>
              <th class="num" style="width:90px">{{ t('sqlList.colQueries') }}</th>
              <th>{{ t('sqlList.colDescription') }}</th>
              <th>{{ t('sqlList.colPath') }}</th>
              <th style="width:120px">{{ t('loadState.loaded') }}</th>
              <th class="num col-when" style="width:140px">{{ t('sqlList.colCreated') }}</th>
          ...
          <tbody>
            ...
            <tr v-else-if="!filteredRows.length">
              <td colspan="9" class="text-center text-secondary py-4">
                ...
                <template v-else>{{ t('sqlList.empty') }}
            ...
            <tr v-for="r in filteredRows" :key="r.id">
              <td>
                <!-- ★ v1.29.1 — [내 것] 배지는 파일명 옆이 자연스럽다 (컨트롤러·서비스와 같은 자리) -->
                <code class="small"><strong>{{ r.name }}.sql</strong></code>
              ...
              <td><span class="text-secondary small">{{ r.table_name || '-' }}</span></td>
              <td class="num">
                <!-- ★ v1.29.0 — 이 파일에 든 SQL 이름 개수 (서버가 query_count 로 준다) -->
                <span class="badge bg-secondary" :title="(r.queries || []).join(', ')">{{ r.query_count ?? (r.queries?.length ?? 0) }}</span>
              </td>
              <td class="small">{{ r.description }}</td>
              <td class="small text-secondary path-cell" :title="r.file_path || ''"><code class="small">{{ r.file_path || '—' }}</code></td>
              <td>
                <LoadStateCell :kind="'sql'" :name="r.name"
