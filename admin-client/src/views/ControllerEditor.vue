...
const title = computed(() => isEdit.value ? '컨트롤러 수정' : '신규 컨트롤러');
...
function generate5DefaultRoutes() {
  ...
  if (added === 0 && skipped > 0) {
    error.value = `5개 기본 라우트가 모두 이미 존재합니다 (중복 ${skipped}개 건너뜀)`;
  }
}
...
async function startRtTest() {
  ...
  rtLog('sys', `연결 시도: ${url.replace(/ticket=[^&]*/, 'ticket=***')}`);
  ...
  rtEs.onerror = () => {
    rtTest.status = 'error';
    rtLog('err', '연결 실패/끊김 — 컨트롤러를 먼저 [저장] 했는지, 인증이 필요한지 확인하세요.');
    stopRtTest(true);
  };
