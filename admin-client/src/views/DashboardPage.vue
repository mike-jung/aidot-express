...
function openConfirm(name) {
  ...
  const cfg = {
    ...
    restart: { title: t('dash2.restartTitle'), btnLabel: t('dash2.restart'), btnClass: 'btn-warning',
               message: t('designer.dsh_restartAsk') },
  }[name];
...
function closeConfigEditor() {
  ...
  if (anyDirty && !window.confirm(t('designer.dsh_unsavedAsk'))) return;
...
async function saveConfigTab() {
  ...
  if (f.content === f.originalContent) {
    configEditor.value.saveMsg = t('designer.dsh_noChange');
    return;
  }
...
async function refreshDbHealth() {
  ...
  try {
    ...
    dbHealth.value = { ok: false, error: e.response?.data?.message || e.message || t('designer.dsh_unknownErr') };
  } finally {
...
const dbCardTooltip = computed(() => {
  ...
  const lines = [];
  if (h.configuredType) lines.push(`${t('designer.dsh_configuredDb')}: ${h.configuredType}`);
