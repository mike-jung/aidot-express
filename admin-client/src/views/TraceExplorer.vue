...
async function runRetention() {
  ...
  } catch (e) { notifyError('보관 정리 실패', e); }
}

function failReason(r) {
  const sm = String(r.summary || '');
  const m = sm.match(/[—-]\s*(.{3,80})$/);
  if (m) return m[1];
  const byCode = {
    400: '요청 형식 오류', 401: '로그인 필요', 403: '권한 없음',
    404: '주소 없음', 409: '충돌', 500: '서버 오류',
  };
...
async function lookup(id) {
  ...
  try {
    ...
    detail.value = null;
    notifyError('요청 기록을 찾을 수 없습니다', e);
  } finally {
    loading.value = false;
  }
}
...
const ACTIVITY_KINDS = [
  { value: 'business', label: '컨트롤러 요청', hint: '업무 API (/api/*)' },
              ...
              <tbody>
                ...
                <tr v-for="r in rows" :key="r.request_id"
                  ...
                  <td class="cell-sub text-truncate" style="max-width:260px">
                    ...
                    <span v-else class="text-secondary">{{ r.summary || '' }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="alert alert-light border mb-3 py-2">
              <div class="fw-semibold">{{ detail.summary }}</div>
