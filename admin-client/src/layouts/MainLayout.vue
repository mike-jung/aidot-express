...
/* ★ v1.39.0 — 실행 방식 배지 */
const runMode = computed(() => uiFlags.flags?.runMode || null);
const runModeIcon = computed(() => ({
  installed: 'bi-box-seam',   /* 설치본 — 상자 */
  release:   'bi-rocket',     /* 배포 빌드 */
  dev:       'bi-code-slash', /* 개발 */
}[runMode.value?.id] || 'bi-question-circle'));
  ...
  <div class="app-shell">
    ...
    <div class="main">
      <div class="topbar">
        ...
        <!-- ★ v1.39.0 — 지금 어떤 방식으로 도는지.
             설치본 서버를 켜 둔 채 콘솔만 npm start 로 띄우면 화면이 똑같아
             어느 쪽을 보고 있는지 알 수 없다. 그것 때문에 실제로 한참 헤맸다. -->
        <span v-if="runMode" class="run-badge" :class="`run-${runMode.id}`" :title="runMode.hint">
          <i class="bi" :class="runModeIcon"></i>{{ runMode.label }}
        </span>
...
<style scoped>
/* ★ v1.39.0 — 실행 방식 배지 (제목 옆)
   세 가지를 색으로 구분한다. 눈에 띄되 제목을 가리지 않을 만큼만. */
.run-badge {
  display: inline-flex;
  align-items: center;
  gap: .35rem;
  margin-left: .75rem;        /* 제목과 붙지 않게 */
  margin-right: auto;         /* 나머지는 오른쪽으로 밀어 둔다 */
  padding: .2rem .55rem;
...
.run-badge .bi { font-size: .8rem; }

/* 설치본 — 이 화면이 설치된 앱의 것이라는 뜻 */
.run-installed { background: #e7f1ff; border-color: #b6d4fe; color: #0a58ca; }
/* 배포 빌드를 직접 실행 */
.run-release   { background: #e8f6ec; border-color: #b7e0c3; color: #146c43; }
/* 개발 — 가장 자주 보게 되므로 조용한 색으로 */
.run-dev       { background: #f3f4f6; border-color: #dfe3ea; color: #5a6270; }

@media (prefers-color-scheme: dark) {
  .run-installed { background: #12294a; border-color: #1d3f6e; color: #8fc0ff; }
  .run-release   { background: #12301f; border-color: #1d4d31; color: #7fd6a0; }
  .run-dev       { background: #1d2432; border-color: #2b3446; color: #9aa1b2; }
}

/* 좁은 화면에서는 글자를 감추고 아이콘만 — 제목이 밀리면 안 된다 */
@media (max-width: 640px) {
  .run-badge { margin-left: .5rem; padding: .2rem .35rem; font-size: 0; gap: 0; }
  .run-badge .bi { font-size: .85rem; }
}
