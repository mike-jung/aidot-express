...
export function genScaffold(project) {
...
function genReadme(project) {
  const name = project.name || '생성된 프로젝트';
  const isMetronic = project.config?.cssFramework === 'metronic';

  // Phase 28 (patch-07): Metronic 번들 배치 안내를 README 에 추가.
  //  이전에는 zip 을 받아 실행해도 스타일이 적용되지 않는 원인을 파악하기 어려웠음.
  const metronicSection = isMetronic ? `
## ⚠️ Metronic 번들 배치 (필수)
...
- 생성된 apiClient 는 \`${project.config?.apiBaseUrl || 'http://localhost:7901'}\` 로 요청합니다.
...
`;
  return { path: 'README.md', content, source: 'scaffold' };
}
