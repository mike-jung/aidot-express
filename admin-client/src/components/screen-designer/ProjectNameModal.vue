...
const title = (props.mode === 'rename') ? '프로젝트 이름 변경' : '새 프로젝트';
const submitLabel = (props.mode === 'rename') ? '변경' : '생성';

async function onSubmit() {
  const trimmed = name.value.trim();
  if (!trimmed) {
    localError.value = '이름을 입력하세요';
    return;
  }
  ...
  try {
    ...
    if (status === 409 && body?.existingId) {
      const ok = await confirmDialog({
        ...
        message: `프로젝트 "${body.existingName || trimmed}" 이(가) 이미 존재합니다.\n기존 프로젝트로 이동할까요?`,
        detail: `ID: ${body.existingId}\n취소하면 다른 이름으로 다시 입력할 수 있습니다.`,
      ...
      localError.value = `"${trimmed}" 은(는) 이미 존재합니다. 다른 이름을 입력하세요.`;
    } else {
