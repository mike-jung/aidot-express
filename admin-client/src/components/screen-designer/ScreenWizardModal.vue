...
function hintFor(inp) {
  ...
  if (source === 'path')  return t('designer.valueIn').replace('{type}', type);
  if (source === 'query') return t('designer.valueQuery').replace('{type}', type);
  return t('designer.valuePlain').replace('{type}', type);
}
...
async function selectController(ctrl) {
  ...
  try {
    ...
    controllerError.value = `${t('designer.ctrlDetailFailed')}: ${e.response?.data?.message || e.message}`;
  }
}
...
async function runAnalyze() {
  ...
  try {
    ...
    notifyError(t('designer.analyzeFailed'), e);
  } finally {
...
async function runProbe() {
  ...
  if (isMutation.value) {
    const ok = await confirmDialog({
      title: t('designer.runRouteTitle').replace('{m}', selectedRoute.value.method),
      message: t('designer.runRouteMsg').replace(/\{m\}/g, selectedRoute.value.method),
      detail: t('designer.runRouteDetail'),
...
function createScreenFromProbe() {
  ...
  if (isMutation) {
    ...
    const actionLabel = method === 'POST' ? t('designer.actCreate')
                     : method === 'DELETE' ? t('designer.actDelete')
                     : t('designer.actUpdate');
    dialogWidget.config = {
