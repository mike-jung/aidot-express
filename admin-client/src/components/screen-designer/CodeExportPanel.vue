...
function regenerate() {
  genError.value = null;
  try {
    rawFiles.value = generateProject(props.project, locale.value);
  } catch (e) {
...
async function clearAllEdits() {
  ...
  if (!await confirmDialog({ title: t('designer.resetEdits'), message: `이 프로젝트의 편집 ${editedPaths.value.size}개를 모두 초기화할까요?`, detail: '되돌릴 수 없습니다.', confirmText: '초기화', variant: 'danger' })) return;
...
<template>
  <div class="code-export-panel">
    ...
    <div class="export-header d-flex justify-content-between align-items-start mb-3">
      <div>
        ...
        <div class="small text-secondary">
          {{ t('codeExportPanel.k2') }}<strong>{{ files.length }}</strong> {{ t('designer.fileCount').replace('{n}', '') }} · {{ (totalBytes / 1024).toFixed(1) }} KB)
          <span v-if="editedPaths.size > 0" class="text-warning ms-1">
            · <i class="bi bi-pencil-fill"></i> {{ t('designer.editedCount').replace('{n}', editedPaths.size) }}
          </span>
        </div>
      </div>
    ...
    <div v-if="genError" class="alert alert-danger small">
      <i class="bi bi-exclamation-triangle me-1"></i>{{ t('designer.genFailed') }}: {{ genError }}
    </div>
    <div v-if="downloadError" class="alert alert-danger small">
      <i class="bi bi-exclamation-triangle me-1"></i>{{ t('designer.downloadFailed') }}: {{ downloadError }}
    ...
    <div v-if="!genError" class="export-split">
      ...
      <div class="file-tree">
        <div class="tree-title d-flex justify-content-between">
          <span>{{ t('designer.files') }} ({{ files.length }})</span>
        </div>
        <TreeNode
  ...
  setup(props, { emit }) {
    return () => {
      ...
      return h('button', {
        ...
        h('span', { class: 'tree-name' }, node.name),
        edited ? h('span', { class: 'edit-dot ms-auto', title: t('designer.editedMark') }, '●') : null,
      ]);
    };
  },
});
