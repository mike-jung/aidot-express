  ...
  try {
    ...
    if (c.status === 'fulfilled') {
      for (const r of (c.value.data?.data || [])) {
        out.push({ kind: '컨트롤러', name: r.name, sub: r.base_path || r.basePath || '',
          to: { name: 'controllers' } });
      }
    }
    if (s.status === 'fulfilled') {
      for (const r of (s.value.data?.data || [])) {
        out.push({ kind: '서비스', name: r.name, sub: r.sql_file || r.sqlFile || '',
          to: { name: 'services' } });
      }
    }
...
const items = computed(() => {
  ...
  const menuItems = props.menu.map((m) => ({
    kind: '화면', name: m.label, sub: m.path, icon: m.icon || 'bi-arrow-right-short', to: { name: m.name },
  }));
...
<template>
  <Teleport to="body">
    <div v-if="open" class="cmdk-backdrop" @mousedown.self="emit('close')">
      <div class="cmdk-panel" role="dialog" aria-modal="true" aria-label="빠른 이동">
