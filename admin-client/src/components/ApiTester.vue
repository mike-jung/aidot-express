    ...
    <div ref="modalRef" class="app-modal" style="max-width:1100px">
      <div ref="headerRef" class="modal-header">
        <h5 class="mb-0">
          ...
          {{ t('designer.api_title') }} — <code class="text-dark">{{ controller.name }}</code>
      ...
      <div class="modal-body">
        <div v-if="!routes.length" class="alert alert-warning">
          {{ t('designer.api_noRoutes') }}
        </div>
        <div v-else>
          <div class="mb-3">
            <label class="form-label small">{{ t('designer.api_pickRoute') }}</label>
          ...
          <div v-if="mciPrefill" class="alert alert-info py-2 small mb-3">
            ...
            <strong>{{ t('designer.api_eaiCtrl') }}</strong> — {{ t('designer.api_eaiBody') }}
            {{ t('designer.api_eaiFields') }}
            {{ t('designer.api_eaiSend') }}
          </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-sm btn-outline-secondary" @click="$emit('close')">{{ t('designer.api_close') }}</button>
      </div>
    </div>
  </div>
</template>
