...
async function load() {
  ...
  try {
    ...
    if (!data.value) error.value = t('designer.trc_noRecord');
  } catch (e) {
...
const KIND_STYLE = {
  controller: { color: '#3b5bdb', icon: 'bi-diagram-3', label: t('designer.trc_controller') },
  service: { color: '#6f42c1', icon: 'bi-gear', label: t('designer.trc_service') },
  sql: { color: '#2f9e44', icon: 'bi-database', label: 'SQL' },
  mci: { color: '#f59f00', icon: 'bi-plug', label: 'EAI' },
  error: { color: '#e03131', icon: 'bi-exclamation-octagon', label: t('designer.trc_error') },
  note: { color: '#94a3b8', icon: 'bi-sticky', label: t('designer.trc_note') },
};
