const k=[{id:"stat",label:"Stat",description:"숫자 하나 큼직하게 (label + value)",category:"data",icon:"stat",allowedSources:["storeCompute","storeState","customVar","endpoint"],defaultConfig:{format:"number",color:"primary"}},{id:"list",label:"List",description:"작은 테이블 (rows 배열 렌더)",category:"data",icon:"list",allowedSources:["endpoint","storeState","customVar"],rowClickable:!0,defaultConfig:{maxRows:5,columns:null}},{id:"listPaged",label:"List (Paged)",description:"페이지네이션 있는 테이블 (맨앞/이전/숫자/다음/맨뒤)",category:"data",icon:"list-paged",allowedSources:["endpoint"],rowClickable:!0,defaultConfig:{perPage:10,columns:null}},{id:"detail",label:"Detail",description:"key-value 데이터 (object 렌더)",category:"data",icon:"detail",allowedSources:["storeState","customVar","endpoint"],defaultConfig:{fields:null}},{id:"text",label:"Text value",description:"한 줄 값 표시 (label + 작은 텍스트)",category:"data",icon:"text",allowedSources:["storeState","storeCompute","customVar"],defaultConfig:{format:"auto"}},{id:"markdown",label:"Markdown",description:"정적 텍스트 / 섹션 설명",category:"data",icon:"markdown",allowedSources:[],defaultConfig:{body:""}},{id:"chart",label:"Chart",description:"차트/그래프 (막대/라인/파이)",category:"viz",icon:"chart",allowedSources:["endpoint","storeState","customVar"],defaultConfig:{chartType:"bar"},fallbackKind:"list"},{id:"progress",label:"Progress",description:"진행률 바 (0~100%)",category:"viz",icon:"progress",allowedSources:["storeState","storeCompute","customVar"],defaultConfig:{max:100},fallbackKind:"stat"},{id:"timeline",label:"Timeline",description:"시간순 이벤트 목록",category:"viz",icon:"timeline",allowedSources:["endpoint","storeState","customVar"],rowClickable:!0,defaultConfig:{},fallbackKind:"list"},{id:"form",label:"Form",description:"입력 폼 (필드 기반 신규/수정)",category:"action",icon:"form",allowedSources:["endpoint"],defaultConfig:{fields:[]},fallbackKind:"detail"},{id:"queryForm",label:"Query Form",description:'입력 필드 + "조회" 버튼 — 같은 row 의 target widget 을 트리거',category:"action",icon:"form-query",allowedSources:["endpoint"],defaultConfig:{fields:[],submitLabel:"조회",targetWidgetId:null}},{id:"formDialog",label:"Form Dialog",description:"버튼 클릭 → 모달 폼 → 제출 → 실행 (create/update/delete)",category:"action",icon:"form-dialog",allowedSources:["endpoint"],defaultConfig:{buttonLabel:"실행",buttonVariant:"primary",dialogTitle:"",fields:[],confirmBeforeSubmit:!1,refreshTargetWidgetId:null}},{id:"button",label:"Action Button",description:"누르면 다른 화면으로 이동하는 버튼 (눌렀을 때 동작에서 설정)",category:"action",icon:"button",allowedSources:["endpoint"],defaultConfig:{label:"버튼",variant:"primary"},fallbackKind:"markdown",rowClickable:!0},{id:"search",label:"Search Bar",description:"검색어 입력 + 필터",category:"action",icon:"search",allowedSources:[],defaultConfig:{placeholder:"검색어 입력…"},fallbackKind:"text"},{id:"image",label:"Image",description:"이미지 표시",category:"media",icon:"image",allowedSources:["storeState","customVar"],defaultConfig:{alt:""},fallbackKind:"markdown"}];function b(e){return k.find(t=>t.id===e)||null}const w=[{id:"page-title",label:"기본 타이틀",description:"제목 + 부제목 (가장 단순)"},{id:"crumbs",label:"경로 + 타이틀",description:"브레드크럼 + 제목"},{id:"stat-hero",label:"히어로 + 통계",description:"큰 숫자 통계를 포함한 헤더"},{id:"tabs",label:"탭 바",description:"상단에 탭 네비게이션"},{id:"none",label:"제목 없음",description:"헤더 영역 생략"}];function _(e){return w.find(t=>t.id===e)||w[0]}let h=Date.now();function c(e="id"){return h++,`${e}_${h.toString(36)}`}function y(e){const t=[],o=new Set,a=/(?:^|\/)(?::([A-Za-z_][A-Za-z0-9_]*)(\?)?|\{([A-Za-z_][A-Za-z0-9_]*)\})/g;let r;for(;(r=a.exec(String(e||"")))!==null;){const l=r[1]||r[3];!l||o.has(l)||(o.add(l),t.push({name:l,required:!r[2]}))}return t}function C(e){return Array.isArray(e==null?void 0:e.params)&&e.params.length?e.params:y(e==null?void 0:e.path)}function P({title:e="새 화면",path:t="/screen"}={}){return{id:c("composite"),kind:"composite",title:e,path:t,header:{kind:"page-title",title:e,subtitle:""},rows:[],params:y(t),customVars:[],customFns:[]}}function S({action:e="none",target:t="",params:o={}}={}){return{action:e,target:t,params:{...o}}}const W=[{id:"none",label:"아무 것도 안 함",description:"클릭해도 반응하지 않습니다"},{id:"navigate",label:"화면 이동",description:"같은 프로젝트의 다른 화면으로 넘어갑니다"}];function I(e){var t;return!!((t=b(e))!=null&&t.rowClickable)}function R(e){const t=/^row\.([\w$]+)$/.exec(String(e||""));return t?t[1]:null}function E(e,{title:t,path:o}={}){const a=P({title:t,path:o});switch(a.screenKind=e,e){case"list":a.rows.push(m("list",{title:t||"목록"}));break;case"detail":{const r=p({kind:"detail"});r.title=t?t+" 결과":"상세";const l=p({kind:"queryForm"});l.title=t?t+" 조회":"조회",l.config={fields:[{name:"id",label:"ID",type:"number",required:!0,default:""}],submitLabel:"조회",targetWidgetId:r.id},a.rows.push({id:c("row"),widths:[12],widgets:[l],style:{...f}}),a.rows.push({id:c("row"),widths:[12],widgets:[r],style:{...f}});break}case"form-new":{const r=p({kind:"formDialog"});r.title=t||"신규 등록",r.config={buttonLabel:"+ 신규 등록",buttonVariant:"success",dialogTitle:t||"신규 등록",fields:[],confirmBeforeSubmit:!1,refreshTargetWidgetId:null},a.rows.push({id:c("row"),widths:[12],widgets:[r],style:{...f}});break}case"form-edit":{const r=p({kind:"formDialog"});r.title=t||"수정",r.config={buttonLabel:"수정",buttonVariant:"primary",dialogTitle:t||"수정",fields:[],confirmBeforeSubmit:!1,refreshTargetWidgetId:null};const l=p({kind:"formDialog"});l.title="삭제",l.config={buttonLabel:"삭제",buttonVariant:"danger",dialogTitle:"삭제 확인",fields:[{name:"id",label:"ID",type:"number",required:!0,default:""}],confirmBeforeSubmit:!0,refreshTargetWidgetId:null};const n=p({kind:"detail"});n.title=t?t+" — 현재 값":"현재 값",a.rows.push({id:c("row"),widths:[12],widgets:[n],style:{...f}}),a.rows.push({id:c("row"),widths:[6,6],widgets:[r,l],style:{...f}});break}case"dashboard":a.rows.push(v([3,3,3,3],[{kind:"stat",title:"지표 1",config:{color:"primary"}},{kind:"stat",title:"지표 2",config:{color:"success"}},{kind:"stat",title:"지표 3",config:{color:"warning"}},{kind:"stat",title:"지표 4",config:{color:"danger"}}])),a.rows.push(v([8,4],[{kind:"chart",title:"추이"},{kind:"list",title:"최근"}]));break;case"kanban":a.rows.push(v([4,4,4],[{kind:"list",title:"To Do"},{kind:"list",title:"In Progress"},{kind:"list",title:"Done"}]));break;case"calendar":a.rows.push(m("markdown",{title:"캘린더",config:{body:`# 월별 일정

캘린더 widget 은 Phase 24 확장 예정. 현재는 placeholder.`}}));break;case"chart":a.rows.push(v([4,4,4],[{kind:"stat",title:"총계"},{kind:"stat",title:"평균"},{kind:"stat",title:"최대"}])),a.rows.push(m("chart",{title:"분포"}));break;case"report":a.rows.push(m("markdown",{title:"리포트 제목",config:{body:"리포트 개요 / 요약문…"}})),a.rows.push(m("list",{title:"상세 내역"}));break;case"empty":break;default:console.warn(`[compositeSchema] createCompositeSpecForKind: unknown kind "${e}", falling back to empty composite. Expected one of: list | detail | form-new | form-edit | dashboard | kanban | calendar | chart | report | empty`);break}return a}function m(e,{title:t="",config:o={}}={}){const a={id:c("row"),widths:[12],widgets:[p({kind:e})],style:{...f}};return t&&(a.widgets[0].title=t),o&&(a.widgets[0].config={...a.widgets[0].config,...o}),a}function v(e,t){if(e.length!==t.length)throw new Error("widths 와 widgetSpecs 길이 불일치");const o=t.map(a=>{const r=p({kind:a.kind});return a.title&&(r.title=a.title),a.config&&(r.config={...r.config,...a.config}),r});return{id:c("row"),widths:[...e],widgets:o,style:{...f}}}function q({widths:e=[12],widgets:t=null}={}){if(!N(e))throw new Error(`Row widths 는 합이 12 여야 합니다; 받은 값 [${e.join(",")}]`);let o;if(Array.isArray(t)){if(t.length!==e.length)throw new Error(`Row 의 widgets 개수 (${t.length}) 와 widths 개수 (${e.length}) 가 일치해야 합니다.`);o=t}else o=e.map(()=>p({kind:"text"}));return{id:c("row"),widths:[...e],widgets:o,style:{...f}}}const f=Object.freeze({gap:16,padding:0,bgColor:"transparent"});function D(e){return e&&(e.style={...f,...e.style||{}},e)}function p({kind:e="text"}={}){const t=b(e)||b("text");return{id:c("wdg"),kind:t.id,title:"",source:null,config:{...t.defaultConfig},style:{...x},...t.rowClickable?{onRowClick:S()}:{}}}const x=Object.freeze({height:"auto",padding:16,border:!0,shadow:"none",bgColor:"#ffffff"}),A=[{id:"none",label:"없음"},{id:"sm",label:"약함"},{id:"md",label:"중간"}];function j(e){return e&&(e.style={...x,...e.style||{}},e)}const z=[{label:"1분할",widths:[12]},{label:"2분할 균등",widths:[6,6]},{label:"3분할 균등",widths:[4,4,4]},{label:"2:1",widths:[8,4]},{label:"1:2",widths:[4,8]},{label:"1:2:1",widths:[3,6,3]},{label:"4분할 균등",widths:[3,3,3,3]}];function N(e){if(!Array.isArray(e)||e.length===0||e.length>6)return!1;for(const t of e)if(!Number.isInteger(t)||t<1||t>12)return!1;return e.reduce((t,o)=>t+o,0)===12}function K(e,t=null){const o=[...e];if(o.length>=6)return o;let a=t??Math.max(1,Math.floor(12/(o.length+1)));const r=o.reduce((i,d)=>i+Math.max(0,d-1),0);if(a=Math.min(a,r,12-o.length),a<1)return o;let l=a;const n=o.map((i,d)=>({w:i,i:d})).sort((i,d)=>d.w-i.w);let s=0;for(;l>0;){const i=n[s%n.length];if(o[i.i]>1&&(o[i.i]--,l--),s++,s>1e3)break}return o.push(a),o}function V(e,t){if(e.length<=1)return e;const o=e[t],a=e.filter((n,s)=>s!==t);let r=o,l=Math.min(t,a.length-1);for(;r>0;)a[l%a.length]++,r--,l++;return a}function L(e,t,o){const a=[...e],r=a[t];return a[t]=a[o],a[o]=r,a}function T(e){const t=(e==null?void 0:e.config)||{},o=t.columns||t.fields||t.keys,a=Array.isArray(o)?o.map(r=>typeof r=="string"?r:(r==null?void 0:r.key)||(r==null?void 0:r.name)).filter(Boolean):[];return[...new Set(["id",...a])]}function O(e){var s;const t=[];if(!e)return t;const a=(e.rows||[]).flatMap(i=>i.widgets||[]);if(!a.length)return t;const r=/:[A-Za-z_]\w*/.test(String(e.path||"")),l=new Set(["detail","form","formDialog","queryForm"]);a.some(i=>l.has(i.kind))&&!r&&t.push({level:"warn",message:"이 화면은 항목 하나를 다루는데 경로에 :id 같은 파라미터가 없습니다. 목록에서 값을 넘길 통로가 없어 빈 화면이 됩니다."});for(const i of a){if(!i.source){t.push({level:"warn",widgetId:i.id,message:`'${i.title||i.kind}' 에 데이터 소스가 없습니다. 카드의 [데이터 소스] 딱지를 눌러 API 를 고르세요.`});continue}["queryForm","formDialog","form"].includes(i.kind)&&((Array.isArray((s=i.config)==null?void 0:s.fields)?i.config.fields:[]).filter(g=>(g==null?void 0:g.name)&&g.name!=="id").length||t.push({level:"warn",widgetId:i.id,message:`'${i.title||i.kind}' 에 입력란이 없습니다. [데이터 소스] 딱지를 눌러 API 를 다시 고르면 입력란을 다시 분석합니다. 그래도 비어 있으면 그 API 가 받는 입력이 없거나, 서비스에서 SQL 을 따라가지 못한 경우입니다.`}))}return t}function F(e,t=[],o=null){var l;const a=e==null?void 0:e.onRowClick,r=[];if(!a||a.action==="none")return r;if(!I(e.kind))return r.push({level:"warn",message:`'${e.kind}' 위젯에는 행이 없어 클릭 설정이 동작하지 않습니다. 지워도 됩니다.`}),r;if(a.action==="navigate"){if(!a.target)return r.push({level:"error",message:"이동할 화면을 고르세요."}),r;if(o&&a.target===o)return r.push({level:"error",message:"자기 자신으로 이동하면 무한 루프가 됩니다."}),r;const n=t.find(i=>i.id===a.target);if(!n)return r.push({level:"error",message:`이동할 화면을 찾을 수 없습니다 (${a.target}). 화면이 지워졌을 수 있습니다.`}),r;for(const i of C(n)){if(!i.required)continue;((l=a.params)==null?void 0:l[i.name])||r.push({level:"error",message:`'${n.title||n.id}' 화면이 요구하는 '${i.name}' 이(가) 비어 있습니다.`})}const s=T(e);for(const[i,d]of Object.entries(a.params||{})){if(!d)continue;const u=R(d);u?s.length&&!s.includes(u)&&r.push({level:"warn",message:`'${i}' ← row.${u} — 이 위젯의 열 목록에 '${u}' 가 없습니다. 오타가 아닌지 확인하세요.`}):r.push({level:"error",message:`'${i}' 의 값 '${d}' 은 row.필드 형식이어야 합니다.`})}}return r}const M=`
// ============================ 공용 유틸 ============================
...
async function fetchEndpoint(endpoint, queryParams, rowContext) {
  ...
  if (missing) {
    const e = new Error('입력 파라미터 대기 중');
...
const ListWidget = {
  ...
  template: \`<div class="card list-widget">
    <div class="card-header d-flex align-items-center">
      ...
      <span v-if="total > 0" class="badge badge-light-primary ms-2">총 {{ total }}건</span>
      <button class="btn-refresh" @click="reload()" title="새로고침">⟳</button>
    </div>
    <div class="card-body">
      <div v-if="awaitingInput" class="text-muted py-4 text-center">
        <i class="bi bi-cursor"></i> 위의 조회 폼에서 값을 입력하세요.
      </div>
      ...
      <div v-if="paginated && totalPages > 1" class="pager">
        <button :disabled="!hasPrev" @click="goPage(page - 1)">◀ 이전</button>
        <span class="pager-info">{{ page }} / {{ totalPages }}</span>
        <button :disabled="!hasNext" @click="goPage(page + 1)">다음 ▶</button>
      </div>
    </div>
  </div>\`,
};
...
`;export{f as D,w as H,z as R,A as S,$ as W,P as a,p as b,E as c,q as d,M as e,k as f,_ as g,x as h,b as i,W as j,C as k,I as l,S as m,D as n,j as o,y as p,K as q,T as r,V as s,L as t,N as u,F as v,O as w};
