...
test('요청 한 건이 사람이 읽는 한 문장으로 요약된다', () => {
  const s = traceStore.describe(recOf());
...
test('상태코드가 결과 문구로 번역된다', () => {
  assert.match(traceStore.describe(recOf({ status: 500 })), /서버 오류 \(500\)/);
  assert.match(traceStore.describe(recOf({ status: 401 })), /인증 실패/);
  assert.match(traceStore.describe(recOf({ status: 403 })), /권한 없음/);
  assert.match(traceStore.describe(recOf({ status: 404 })), /대상 없음/);
});

test('메서드가 동사로 번역된다 — DELETE 는 "삭제했습니다"', () => {
  assert.match(traceStore.describe(recOf({ method: 'DELETE' })), /삭제했습니다/);
  assert.match(traceStore.describe(recOf({ method: 'POST' })), /만들었습니다/);
  assert.match(traceStore.describe(recOf({ method: 'PUT' })), /수정했습니다/);
});

test('로그인하지 않은 요청은 "익명 사용자" 로 표현된다', () => {
  assert.match(traceStore.describe(recOf({ anonymous: true })), /익명 사용자가/);
});
...
test('요약 문장에 "을(를)" 같은 표기가 남지 않는다', () => {
  for (const p of ['/api/admin/controllers', '/api/admin/users', '/api/auth/login']) {
    const s = traceStore.describe(recOf({ path: p }));
