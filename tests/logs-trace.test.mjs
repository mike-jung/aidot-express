...
test('⑤ describe: 하위 행위 동사', () => {
  ...
  assert.match(traceStore.describe({ ...base, method: 'POST', path: '/api/admin/sqls/test' }), /시험 실행했습니다/);
  assert.match(traceStore.describe({ ...base, method: 'POST', path: '/api/admin/sqls' }), /만들었습니다/);
  assert.match(traceStore.describe({ ...base, method: 'POST', path: '/api/admin/logs/download' }), /내려받았습니다/);
  assert.match(traceStore.describe({ ...base, method: 'DELETE', path: '/api/admin/controllers/:id' }), /삭제했습니다/);
});
